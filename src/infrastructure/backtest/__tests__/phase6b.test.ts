...
function makeInds(n: number, opts: { atr?: (number | undefined)[] } = {}): PrecomputedIndicators {
  ...
  return {
    ...
    stoch: def,
    wma: def, vwma: def, cci: def, williamsR: def,
    momentum: def, obv: def, volumeRatio: def,
    hma: def, dema: def,
    ichimoku: noIchimoku(), donchian: noDonchian(), keltner: noDonchian(),
    stochRsi: def, roc: def, ao: def, aroon: noAroon(),
    forceIndex: def, mfi: def, cmf: def, psar: def,
