...
export function DashboardOS() {
  ...
  const voiceAmpVal = voice.status === "speaking" ? 0.65 : voice.status === "listening" ? 0.35 : 0;

  // =================================================================
  // レンダリング — Full-Screen AI Command Environment v5.0
  // =================================================================
  return (
    <div className="relative h-full w-full overflow-hidden" style={{background:"#020408"}}>

      {/* ░░ LAYER 0 — BrainParticleCore (Full-Screen AI Environment) ░░ */}
      <div className="absolute inset-0 z-0">
        <BrainParticleCore
          brainState={brainState}
          decision={orderProposal?.direction ?? null}
          voiceAmp={voiceAmpVal}
      ...
      <div className="absolute inset-0 z-[3] pointer-events-none">
        <div className="absolute inset-0" style={{background:"radial-gradient(ellipse 80% 60% at 50% 55%, rgba(0,150,230,0.022) 0%, transparent 70%)"}}/>
        <div className="absolute inset-0 avl-grid-bg opacity-[0.025]"/>
        <div className="absolute left-0 right-0 h-px avl-scan-line"
          style={{background:"linear-gradient(to right,transparent,rgba(0,255,136,0.09),transparent)"}}/>
      </div>
      ...
      <div className="absolute top-0 left-0 right-0 z-20 h-11 flex items-center px-5 gap-5"
        style={{
          ...
          boxShadow:"0 1px 28px rgba(0,0,0,0.55)",
        }}>
      ...
      {mode !== "analysis" && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none select-none"
          ...
          <p className="font-black font-mono tracking-[0.5em] transition-all duration-700"
            style={{
              fontSize:26,
              color:"rgba(255,255,255,0.78)",
              textShadow:"0 0 10px rgba(255,255,255,0.18),0 0 40px rgba(0,170,230,0.10)",
            }}>
            AVL AI
          </p>
      ...
      {/* ░░ LAYER 20 — Order proposal (in-canvas) ░░ */}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-30 flex flex-col items-center pb-5 gap-2">
          ...
          <div className="flex items-center gap-3 mb-0.5">
            <div className="flex items-end gap-0.5 h-4 overflow-hidden">
              {[2,4,7,11,15,18,15,11,7,4,2,5,9,13,9,5].map((h,i)=>(
                <div key={i} className="w-0.5 rounded-full"
                  style={{
                    height:Math.min(h,18),
                    backgroundColor:neonHex,
                    opacity:isActive?0.62:0.10,
            ...
            <p className="text-[10px] font-mono tracking-[0.2em] font-semibold"
              style={{color:neonHex,textShadow:`0 0 8px ${neonHex}70`}}>
              {aiStatusText}
            </p>
          ...
          <div className="relative flex items-center justify-center" style={{width:96,height:96,contain:"layout style"}}>
            <div className="absolute rounded-full pointer-events-none"
              style={{
                ...
                boxShadow:`0 0 30px ${neonHex}18,0 0 60px ${neonHex}08`,
            ...
            <div className="absolute rounded-full pointer-events-none"
              style={{
                ...
                background:`radial-gradient(circle,${neonHex}14 0%,transparent 70%)`,
            ...
            <div className="absolute inset-0 rounded-full"
              style={{
                ...
                opacity:isVoiceActive?0.18:0,
                transition:"opacity 0.3s ease",
                willChange:"transform,opacity",
              }}/>
            <div className="absolute w-24 h-24 rounded-full transition-all duration-500"
              style={{border:`1px solid ${isVoiceActive?neonHex+"30":"#141e26"}`,boxShadow:isVoiceActive?`0 0 18px ${neonHex}14`:"none"}}/>
            <div className="absolute w-[70px] h-[70px] rounded-full transition-all duration-500"
              style={{border:`1px solid ${isVoiceActive?neonHex+"50":"#1a2a35"}`,boxShadow:isVoiceActive?`0 0 12px ${neonHex}22`:"none"}}>
              {["absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2",
                ...
                <div key={i} className={`${cls} w-1.5 h-1.5 rounded-full transition-all duration-500`}
                  style={{backgroundColor:isVoiceActive?neonHex:"#1a3040",boxShadow:isVoiceActive?`0 0 6px ${neonHex}`:"none"}}/>
              ))}
            </div>
            <div className="absolute w-[50px] h-[50px] rounded-full transition-all duration-500"
              style={{border:`1px solid ${isVoiceActive?neonHex+"70":"#1a2a35"}`,boxShadow:isVoiceActive?`0 0 8px ${neonHex}30`:"none"}}/>
            <button type="button" disabled={voice.status==="connecting"}
              ...
              style={{
                background:isVoiceActive?`radial-gradient(circle,${neonHex}28 0%,${neonHex}06 70%)`:"radial-gradient(circle,rgba(0,60,100,0.40) 0%,rgba(0,20,40,0.18) 70%)",
                border:`2px solid ${isVoiceActive?neonHex:neonHex+"28"}`,
                boxShadow:isVoiceActive?`0 0 20px ${neonHex}50,0 0 40px ${neonHex}16,inset 0 0 12px ${neonHex}10`:`0 0 6px ${neonHex}14`,
              }}>
              <Mic size={17} style={{color:isVoiceActive?neonHex:`${neonHex}55`}} className={isVoiceActive?"animate-pulse":""}/>
            </button>
          </div>
          ...
          <div className="flex flex-col items-center gap-1">
            <span className="text-[8px] font-mono tracking-[0.35em] transition-all duration-300"
              style={{color:isVoiceActive?neonHex:"#1e2c3a",textShadow:isVoiceActive?`0 0 8px ${neonHex}55`:"none"}}>
            ...
            {isVoiceActive && (
              <div className="flex items-end gap-0.5 h-3">
                {[2,4,6,9,6,4,2,4,6,4,2].map((h,i)=>(
                  <div key={i} className="w-0.5 rounded-full"
                    style={{height:h,backgroundColor:neonHex,opacity:0.72,
