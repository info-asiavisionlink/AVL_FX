...
export function EconomicCalendarPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8f7f4]">
      {/* ヘッダー */}
      <div className="flex items-center gap-1.5 px-2.5 py-2 border-b border-[rgba(0,0,0,0.06)] shrink-0">
            ...
            {upcoming.map(ev => {
              ...
              return (
                <button key={ev.id} onClick={() => router.push("/calendar")}
                  className={cn(
                    "w-full text-left px-2.5 py-1.5 border-b border-[rgba(0,0,0,0.06)]/40 border-l-2 transition-colors",
                    isImm  ? "border-l-red-500 bg-red-950/20" :
                    isNear ? "border-l-amber-500/70 bg-amber-950/10" :
                    isHigh ? "border-l-red-700/60 hover:bg-[rgba(0,0,0,0.04)]/30" :
                             "border-l-transparent hover:bg-[rgba(0,0,0,0.04)]/20"
                  )}>
