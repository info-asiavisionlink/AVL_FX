...
function CopyButton({ value, label }: { value: string; label: string }) {
  ...
  return (
    <button onClick={copy}
      ...
      style={{ background: copied ? "rgba(249,115,22,0.15)" : "rgba(249,115,22,0.1)", color: copied ? "#f97316" : "#f97316", border: `1px solid ${copied ? "rgba(249,115,22,0.3)" : "rgba(249,115,22,0.25)"}` }}>
...
function ValueBox({ label, value, mono = true }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-lg p-3" style={{ background: "#f8f7f4", border: "1px solid rgba(0,0,0,0.06)" }}>
      <p className="text-[9px] tracking-widest mb-1.5" style={{ color: "#4a4a4a" }}>{label}</p>
      <div className="flex items-center justify-between gap-3">
        <p className={`text-xs break-all ${mono ? "font-mono" : ""}`} style={{ color: "#1a1a1a" }}>{value}</p>
    ...
    <div className="flex gap-4">
      <div className="flex flex-col items-center">
        <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-black"
          style={{
            background: done ? "rgba(249,115,22,0.2)" : active ? "rgba(249,115,22,0.15)" : "rgba(0,0,0,0.05)",
            color: done ? "#f97316" : active ? "#f97316" : "#4a4a4a",
            border: `2px solid ${done ? "rgba(249,115,22,0.4)" : active ? "rgba(249,115,22,0.3)" : "rgba(255,255,255,0.1)"}`,
          }}>
          {done ? <CheckCircle size={16} /> : num}
        </div>
        <div className="w-px flex-1 mt-2" style={{ background: "rgba(0,0,0,0.04)" }} />
      </div>
      <div className="flex-1 pb-8">
        <p className="text-sm font-bold mb-3" style={{ color: active || done ? "#1a1a1a" : "#4a4a4a" }}>{title}</p>
...
export function MT5ConnectionPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-black tracking-widest" style={{ color: "#1a1a1a" }}>MT5接続</h1>
      ...
      {hasConnection && (
        <div className="flex items-center gap-3 rounded-xl px-4 py-3"
          style={{ background: isOnline ? "rgba(249,115,22,0.05)" : "rgba(0,0,0,0.03)", border: `1px solid ${isOnline ? "rgba(249,115,22,0.2)" : "rgba(0,0,0,0.06)"}` }}>
          <div className={`w-2.5 h-2.5 rounded-full ${isOnline ? "bg-green-400" : "bg-gray-500"}`}
            style={{ boxShadow: isOnline ? "0 0 6px #4ade80" : "none" }} />
          <div className="flex-1">
            <p className="text-xs font-bold" style={{ color: isOnline ? "#4ade80" : "#9a9a9a" }}>
              {isOnline ? "EA接続中 — 正常稼働中" : "EA未接続 — MT5でEAを起動してください"}
            </p>
            {connection.last_heartbeat_at && (
              <p className="text-[9px] mt-0.5" style={{ color: "#4a4a4a" }}>
      ...
      {/* Steps */}
      <div className="rounded-xl p-6" style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.06)" }}>
        ...
        <Step num={1} title="EAファイルをダウンロード" done={step >= 2} active={step === 1}>
          ...
          <p className="text-[10px] mt-2" style={{ color: "#4a4a4a" }}>
            ダウンロードしたら「次へ」を押してください
          </p>
          {step === 1 && (
            <button onClick={() => setStep(2)} className="mt-2 text-[11px] underline" style={{ color: "#4a4a4a" }}>
              ダウンロード済み → 次へ
            </button>
          )}
        </Step>
        ...
        <Step num={2} title="MT5のフォルダに入れる" done={step >= 3} active={step === 2}>
          <div className="space-y-3">
            ...
            {/* Mac */}
            <div className="rounded-lg overflow-hidden" style={{ border: "1px solid rgba(249,115,22,0.25)" }}>
              <div className="px-3 py-2 flex items-center gap-2" style={{ background: "rgba(249,115,22,0.1)" }}>
                <span className="text-sm">🍎</span>
                <span className="text-xs font-bold" style={{ color: "#f97316" }}>Mac をご利用の場合</span>
              </div>
              <div className="px-3 py-3 space-y-2">
                <p className="text-xs" style={{ color: "#9a9a9a" }}>
                  .ex5 はダブルクリックで開けません。<br />
                  <strong style={{ color: "#1a1a1a" }}>ターミナル</strong>（Launchpad → その他 → ターミナル）を開いて以下をコピペしてください：
                </p>
                <div className="rounded p-2.5" style={{ background: "rgba(0,0,0,0.03)" }}>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-[10px]" style={{ color: "#4a4a4a" }}>「✅ コピー完了」と表示されたら成功</p>
                  <CopyButton
            ...
            <div className="rounded-lg overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>
              <div className="px-3 py-2 flex items-center gap-2" style={{ background: "rgba(0,0,0,0.05)" }}>
                <span className="text-sm">🪟</span>
                <span className="text-xs font-bold" style={{ color: "#9a9a9a" }}>Windows をご利用の場合</span>
              </div>
              <div className="px-3 py-3 space-y-1.5 text-xs" style={{ color: "#9a9a9a" }}>
                <p>① MT5 メニュー →「ファイル」→「データフォルダを開く」</p>
                <p>② <span className="font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(0,0,0,0.04)", color: "#f97316" }}>MQL5 → Experts</span> フォルダを開く</p>
                <p>③ ダウンロードした <span className="font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(0,0,0,0.04)", color: "#f97316" }}>AVL_FX_Bridge.ex5</span> をコピー</p>
              </div>
            </div>

            {/* 共通 */}
            <p className="text-xs" style={{ color: "#4a4a4a" }}>
              📍 Mac・Windows 共通：MT5 ナビゲーターで「エキスパートアドバイザ」を右クリック →「<strong style={{ color: "#1a1a1a" }}>更新</strong>」
            </p>

          </div>
          {step === 2 && (
            <button onClick={() => setStep(3)} className="mt-4 text-[11px] underline" style={{ color: "#4a4a4a" }}>
              配置完了 → 次へ
            </button>
          )}
        </Step>
        ...
        <Step num={3} title="MT5にWebRequestを許可する" done={step >= 4} active={step === 3}>
          ...
          <div className="space-y-2 text-xs" style={{ color: "#9a9a9a" }}>
          ...
          {step === 3 && (
            <button onClick={() => setStep(4)} className="mt-3 text-[11px] underline" style={{ color: "#4a4a4a" }}>
              設定完了 → 次へ
            </button>
          )}
        </Step>
        ...
        <Step num={4} title="接続情報を取得する" done={step >= 5 && hasToken} active={step === 4}>
          ...
          {!hasConnection ? (
            <button onClick={issueToken} disabled={issuing}
              className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all hover:opacity-80"
              style={{ background: "rgba(249,115,22,0.12)", color: "#f97316", border: "1px solid rgba(249,115,22,0.3)", opacity: issuing ? 0.6 : 1 }}>
            ...
            <div className="space-y-2">
              ...
              {hasToken ? (
                <div className="rounded-lg p-3" style={{ background: "rgba(22,163,74,0.06)", border: "1px solid rgba(249,115,22,0.2)" }}>
              ...
              {step === 4 && hasToken && (
                <button onClick={() => setStep(5)} className="mt-1 text-[11px] underline" style={{ color: "#4a4a4a" }}>
                  コピーした → 次へ
                </button>
              )}
            </div>
        ...
        <Step num={5} title="EAをチャートにアタッチ" done={isOnline} active={step >= 5}>
          <div className="space-y-2 text-xs" style={{ color: "#9a9a9a" }}>
            ...
            <p>② ナビゲーターから <span className="font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(0,0,0,0.04)", color: "#f97316" }}>AVL_FX_Bridge</span> をチャートにドラッグ</p>
            <p>③ EA設定画面が開いたら以下の <strong>3つ</strong> を入力：</p>
            <div className="ml-4 space-y-1.5 mt-2 p-3 rounded-lg" style={{ background: "#f8f7f4", border: "1px solid rgba(0,0,0,0.06)" }}>
              <div className="flex gap-2 items-center">
                <span className="font-mono text-[10px] px-2 py-0.5 rounded flex-shrink-0" style={{ background: "rgba(249,115,22,0.08)", color: "#f97316" }}>InpServerURL</span>
                <span style={{ color: "#4a4a4a" }}>← STEP 4 の Gateway URL</span>
              </div>
              <div className="flex gap-2 items-center">
                <span className="font-mono text-[10px] px-2 py-0.5 rounded flex-shrink-0" style={{ background: "rgba(249,115,22,0.08)", color: "#f97316" }}>InpConnectionId</span>
                <span style={{ color: "#4a4a4a" }}>← STEP 4 の Connection ID</span>
              </div>
              <div className="flex gap-2 items-center">
                <span className="font-mono text-[10px] px-2 py-0.5 rounded flex-shrink-0" style={{ background: "rgba(249,115,22,0.08)", color: "#f97316" }}>InpConnectionToken</span>
                <span style={{ color: "#4a4a4a" }}>← STEP 4 の Connection Token（緑文字）</span>
              </div>
            </div>
          ...
          {!isOnline && step >= 5 && (
            <div className="flex items-center gap-2 mt-3">
              ...
              <p className="text-[11px]" style={{ color: "#4a4a4a" }}>EA接続を待っています...</p>
            </div>
          )}
          {isOnline && (
            <div className="flex items-center gap-2 mt-3">
      ...
      {hasConnection && !issuing && (
        <div className="rounded-xl px-5 py-4" style={{ background: "#ffffff", border: "1px solid rgba(0,0,0,0.04)" }}>
          <p className="text-[9px] font-bold tracking-widest mb-2" style={{ color: "#4a4a4a" }}>TOKEN紛失・再接続</p>
