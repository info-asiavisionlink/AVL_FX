...
function OverviewTab({ strategy }: { strategy: StrategyRecord }) {
  ...
  const Row = ({ label, value, color }: { label: string; value: string; color?: string }) => (
    <div className="flex items-start gap-3 py-1.5" style={{ borderBottom: "1px solid rgba(0,0,0,0.03)" }}>
      <span className="text-[8px] font-mono tracking-widest w-28 shrink-0 mt-0.5" style={{ color: "#9a9a9a" }}>
        {label}
      </span>
      <span className="text-[10px] font-mono" style={{ color: color ?? "#9a9a9a" }}>
        {value}
      </span>
    </div>
  );
  ...
  return (
    <div className="p-5 space-y-5 overflow-y-auto">
      ...
      <section>
        ...
        <div className="px-3 py-1 rounded" style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)" }}>
        ...
        <div className="space-y-1.5">
          {(conds as Record<string, unknown>[]).map((c, i) => (
            <div key={i} className="flex items-center gap-2 px-3 py-2 rounded"
              ...
              <span className="text-[9px] font-mono" style={{ color: "#4a4a4a" }}>
                {String(c.operator ?? "")}
      ...
      {exit && Object.keys(exit).length > 0 && (
        <section>
          <p className="text-[8px] font-black tracking-[0.25em] mb-2" style={{ color: "#4a4a4a" }}>決済条件</p>
          <div className="px-3 py-1 rounded" style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)" }}>
            {exit.stop_loss ? (() => {
...
function EquityCurve({ trades }: { trades: DBTrade[] }) {
  ...
  return (
    <div>
      <p className="text-[8px] font-black tracking-[0.22em] mb-2" style={{ color: "#9a9a9a" }}>
        エクイティカーブ — 累積PIPS
      </p>
    ...
    <div className="rounded-lg p-4" style={{
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <div className="text-center px-2 py-2 rounded" style={{ background: `${wrCol}10`, border: `1px solid ${wrCol}25` }}>
          <p className="text-[7px] font-mono tracking-widest mb-0.5" style={{ color: "#4a4a4a" }}>勝率</p>
          <p className="text-[20px] font-black leading-none" style={{ color: wrCol }}>{stats.winRate.toFixed(1)}%</p>
        </div>
        <div className="text-center px-2 py-2 rounded" style={{ background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.04)" }}>
          <p className="text-[7px] font-mono tracking-widest mb-0.5" style={{ color: "#4a4a4a" }}>プロフィットファクター</p>
  ...
  return (
    <div className="rounded overflow-hidden" style={{ border: "1px solid rgba(0,0,0,0.08)" }}>
      ...
      <button
        ...
        style={{ background: open ? "rgba(0,0,0,0.03)" : "rgba(0,0,0,0.02)" }}
        onClick={() => setOpen(v => !v)}
      >
        {/* 展開矢印 */}
        <span className="text-[8px] font-mono shrink-0 transition-transform"
          style={{ color: "#4a4a4a", transform: open ? "rotate(90deg)" : "rotate(0deg)", display: "inline-block" }}>
          ▶
        </span>

        {/* 月ラベル */}
        <span className="text-[10px] font-black font-mono w-24 shrink-0" style={{ color: "#9a9a9a" }}>
          {label}
        </span>

        {/* トレード数 */}
        <span className="text-[9px] font-mono shrink-0" style={{ color: "#4a4a4a" }}>
          {stats.total}件
        </span>
        ...
        {/* PF */}
        <span className="text-[9px] font-mono shrink-0 w-10 text-right" style={{ color: "#4a4a4a" }}>
          PF {pfDisp}
        </span>
      ...
      {/* ドロップダウン: 個別トレード */}
      {open && (
        <div style={{ background: "rgba(0,0,0,0.20)", borderTop: "1px solid rgba(0,0,0,0.05)" }}>
          {/* テーブルヘッダー */}
          <div className="grid px-3 py-1.5" style={{
            ...
            {["エントリー", "決済", "方向", "結果", "PIPS", "理由"].map(h => (
              <span key={h} className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>{h}</span>
            ))}
          </div>
          {/* トレード行 */}
          {trades.map((t, i) => {
            ...
            return (
              <div key={t.id ?? i}
                ...
                {/* 理由 */}
                <span className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>
                  {t.exit_reason}
                </span>
              </div>
            );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="px-4 py-3 rounded-lg"
        ...
        <p className="text-[13px] font-black font-mono" style={{ color: "#1a1a1a" }}>
          {trades.length
        ...
        <div className="flex items-center gap-3 mt-1.5 flex-wrap">
          ...
          <span className="text-[9px] font-mono" style={{ color: "#9a9a9a" }}>
      ...
      {monthKeys.length > 0 && (
        <div className="space-y-2">
          <p className="text-[8px] font-black tracking-[0.22em]" style={{ color: "#4a4a4a" }}>
            月次内訳 — クリックで詳細展開
          </p>
          {monthKeys.map(key => (
    ...
    <div className="space-y-4">
      ...
      <div className="px-4 py-3 rounded"
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[8px] font-mono tracking-widest mb-1" style={{ color: "#4a4a4a" }}>判定</p>
          ...
          <div className="text-right">
            <p className="text-[8px] font-mono tracking-widest mb-1" style={{ color: "#9a9a9a" }}>期間</p>
            <p className="text-[9px] font-mono" style={{ color: "#4a4a4a" }}>{r.periodLabel}</p>
          </div>
        </div>
        {r.verdictReason && (
          <p className="text-[9px] font-mono mt-2 leading-relaxed" style={{ color: "#4a4a4a" }}>
            {r.verdictReason}
          </p>
        )}
      </div>
      ...
      {r.sampleSizeWarning && (
        <div className="flex items-start gap-2 px-3 py-2 rounded"
          ...
          <div>
            ...
            <p className="text-[8px] font-mono mt-0.5" style={{ color: "#4a4a4a" }}>
      ...
      {/* Key metrics */}
      <div>
        <p className="text-[8px] font-black tracking-[0.22em] mb-2" style={{ color: "#9a9a9a" }}>主要指標</p>
        ...
        <div className="px-4 py-3 rounded mb-2"
          ...
          <p className="text-[8px] font-mono tracking-widest mb-1" style={{ color: "#9a9a9a" }}>合計 PIPS</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
          {[
            ...
            { label: "取引数",     value: String(r.totalTrades),               color: "#9a9a9a" },
            ...
            { label: "DDのPIPS",   value: r.maxDrawdownPips.toFixed(1),        color: "#4a4a4a" },
            { label: "勝ち",       value: String(r.wins),                       color: NG },
            { label: "負け",       value: String(r.losses),                    color: RED },
            { label: "平均時間",   value: `${r.avgDuration.toFixed(0)} min`,   color: "#4a4a4a" },
          ].map(({ label, value, color }) => (
            <div key={label} className="px-2 py-2 rounded"
              style={{ background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.05)" }}>
              <p className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>{label}</p>
      ...
      {/* Data coverage */}
      <div className="flex gap-3 text-[8px] font-mono" style={{ color: "#9a9a9a" }}>
              ...
              return (
                <div key={sess} className="flex items-center gap-3 px-3 py-2 rounded"
                  ...
                  <div className="flex items-center gap-1.5 w-28 shrink-0">
                    <span className="text-[9px] font-mono font-bold" style={{ color: "#4a4a4a" }}>{sess}</span>
                  ...
                  <span className="text-[8px] font-mono w-14 shrink-0" style={{ color: "#4a4a4a" }}>
...
function BacktestTab({ strategyId, onJobIdChange }: {
  ...
  return (
    <div className="p-5 space-y-5 overflow-y-auto">
      ...
      {/* Run panel */}
      <div className="p-4 rounded" style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {PERIODS.map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              ...
              style={{
                background:    p === period ? `${CYAN}15` : "rgba(0,0,0,0.03)",
                border:        `1px solid ${p === period ? `${CYAN}45` : "rgba(0,0,0,0.06)"}`,
                color:         p === period ? CYAN : "#4a4a4a",
        ...
        <div className="flex items-center gap-3 mb-4">
          <span className="text-[8px] font-mono tracking-widest shrink-0" style={{ color: "#9a9a9a" }}>
            初期残高
          </span>
          <div className="relative flex-1 max-w-[160px]">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-mono" style={{ color: "#4a4a4a" }}>$</span>
            <input
              type="number"
              value={balance}
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${state === "running" ? "rgba(0,0,0,0.06)" : `${NG_r}0.40)`}`,
            color:  state === "running" ? "#9a9a9a" : NG,
            cursor: state === "running" ? "not-allowed" : "pointer",
          }}>
          {state === "running"
            ? "◌ バックテスト実行中..."
      ...
      {result && (
        <>
          ...
          <div className="px-4 py-3 rounded"
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[8px] font-mono tracking-widest mb-0.5" style={{ color: "#4a4a4a" }}>判定</p>
              ...
              {result.verdictReason && (
                <p className="text-[9px] font-mono leading-relaxed max-w-[55%] text-right" style={{ color: "#4a4a4a" }}>
                  {result.verdictReason}
                </p>
              )}
            </div>
...
function TradesTab({ jobId }: { jobId: string | null }) {
  ...
  if (!jobId) {
    return (
      <div className="p-5 flex items-center justify-center h-48">
        <p className="text-[9px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>
  ...
  return (
    <div className="p-5 space-y-3 overflow-y-auto">
        ...
        <div className="flex gap-1.5">
          {(["ALL", "WIN", "LOSS"] as TradeFilter[]).map(f => (
            <button key={f} onClick={() => { setFilter(f); setPage(0); }}
              ...
              style={{
                background: f === filter ? (f === "WIN" ? `${NG}18` : f === "LOSS" ? `${RED}18` : "rgba(255,255,255,0.07)") : "rgba(0,0,0,0.03)",
                border:     `1px solid ${f === filter ? (f === "WIN" ? `${NG}40` : f === "LOSS" ? `${RED}40` : "rgba(0,0,0,0.08)") : "rgba(0,0,0,0.04)"}`,
                color:      f === filter ? (f === "WIN" ? NG : f === "LOSS" ? RED : "#9a9a9a") : "#4a4a4a",
              }}>
              {f}
            </button>
          ))}
      ...
      {/* Table */}
      {paged.length === 0 ? (
        <p className="text-[9px] font-mono text-center py-8" style={{ color: "#9a9a9a" }}>取引データなし</p>
      ) : (
              ...
              {paged.map((t) => {
                ...
                return (
                  <tr key={t.id} style={{ background: "rgba(255,255,255,0.015)" }}>
                    <td className="px-2 py-1.5 text-[8px] font-mono rounded-l" style={{ color: "#4a4a4a" }}>
                      {fmtTime(t.entry_time)}
                    </td>
                    <td className="px-2 py-1.5 text-[8px] font-mono font-bold" style={{ color: dirCol }}>
                      {t.direction}
                    </td>
                    <td className="px-2 py-1.5 text-[8px] font-mono" style={{ color: "#9a9a9a" }}>
                    ...
                    <td className="px-2 py-1.5 text-[8px] font-mono" style={{ color: "#4a4a4a" }}>
                      {t.session ?? "–"}
                    </td>
                    <td className="px-2 py-1.5 text-[8px] font-mono rounded-r" style={{ color: "#9a9a9a" }}>
                      {t.exit_reason}
                    </td>
                  </tr>
                );
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-1">
          <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
            ...
            style={{ color: "#4a4a4a", opacity: page === 0 ? 0.3 : 1 }}>
            ← 前
          </button>
          <span className="text-[8px] font-mono" style={{ color: "#9a9a9a" }}>
            {page + 1} / {totalPages}
          ...
          <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1}
            className="px-2 py-1 rounded text-[8px] font-mono transition-opacity"
            style={{ color: "#4a4a4a", opacity: page === totalPages - 1 ? 0.3 : 1 }}>
            次 →
          </button>
        </div>
      )}
...
function StrategyChatSection({ strategyId }: { strategyId: string }) {
  ...
  return (
    <div className="space-y-3 pt-4" style={{ borderTop: "1px solid rgba(0,0,0,0.04)" }}>
        ...
        >
              ...
              <div
                ...
                style={{
                  background: m.role === "user"
                    ...
                    : "rgba(0,0,0,0.03)",
                  border: `1px solid ${m.role === "user" ? "rgba(249,115,22,0.25)" : "rgba(0,0,0,0.06)"}`,
                  color: m.role === "user" ? CYAN : "#9a9a9a",
                  whiteSpace: "pre-wrap",
                }}
              >
                {m.role === "assistant" && (
                  <span className="text-[7px] font-black tracking-widest block mb-1" style={{ color: "#4a4a4a" }}>
                    AI
                  </span>
                )}
                {m.content}
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="px-3 py-2 rounded-lg" style={{ background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.06)" }}>
                <span className="text-[9px] font-mono" style={{ color: "#9a9a9a" }}>◌ 考え中...</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          style={{
            background: !input.trim() || loading ? "rgba(0,0,0,0.03)" : `rgba(249,115,22,0.15)`,
            border: `1px solid ${!input.trim() || loading ? "rgba(0,0,0,0.06)" : "rgba(249,115,22,0.40)"}`,
            color: !input.trim() || loading ? "#9a9a9a" : CYAN,
...
function FactReportSection({ strategyId }: { strategyId: string }) {
  ...
  return (
    <div className="p-4 rounded-lg" style={{ background: "rgba(249,115,22,0.03)", border: "1px solid rgba(249,115,22,0.12)" }}>
...
function AnalysisTab({ strategyId, hasBacktest }: { strategyId: string; hasBacktest: boolean }) {
  ...
  if (state === "loading") {
    return (
      <div className="p-5 flex items-center justify-center h-48">
        <p className="text-[9px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>◌ Loading...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-5 space-y-5 overflow-y-auto">
      ...
      <div style={{ borderTop: "1px solid rgba(0,0,0,0.04)", paddingTop: "16px" }}>
        <p className="text-[8px] font-black tracking-[0.25em] mb-3" style={{ color: "#9a9a9a" }}>
          詳細AI分析（オプション）
        </p>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            style={{
              ...
              border:  `1px solid ${state === "running" ? "rgba(0,0,0,0.06)" : "rgba(167,139,250,0.40)"}`,
              color:   state === "running" ? "#9a9a9a" : PURPLE,
              cursor:  state === "running" ? "not-allowed" : "pointer",
            }}>
            {state === "running"
              ? "◌ AI分析中..."
      ...
      {analysis && (
        <div className="space-y-5">
          ...
          <div className="px-4 py-3 rounded"
            ...
            <div className="flex items-start justify-between gap-4 mb-2">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <p className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>信頼度</p>
            ...
            {analysis.data_quality_note && (
              <p className="text-[8px] font-mono mt-2 leading-relaxed" style={{ color: "#4a4a4a" }}>
                ⚠ {analysis.data_quality_note}
              </p>
            )}
          </div>
              ...
              {analysis.facts.map((f, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2 rounded"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-mono leading-relaxed" style={{ color: "#9a9a9a" }}>{f.statement}</p>
                  ...
                  <span className="text-[7px] font-mono shrink-0 mt-0.5" style={{ color: "#9a9a9a" }}>
                    {f.source}
                  </span>
                </div>
              ))}
            ...
            <Section title="観察 — 推測されたパターン" color={AMBER}>
              {analysis.observations.map((o, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2 rounded"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-mono leading-relaxed" style={{ color: "#9a9a9a" }}>{o.observation}</p>
                    <p className="text-[8px] font-mono mt-1 leading-relaxed" style={{ color: "#4a4a4a" }}>
                      根拠: {o.basis}
                    </p>
                  </div>
                  {o.confidence && (
            ...
            <Section title="強み" color={NG}>
              {analysis.strengths.map((s, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2 rounded"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-mono font-bold" style={{ color: "#9a9a9a" }}>{s.point}</p>
                    {s.detail && <p className="text-[8px] font-mono mt-0.5" style={{ color: "#4a4a4a" }}>{s.detail}</p>}
                  </div>
                </div>
              ))}
            </Section>
            ...
            <Section title="弱み" color={RED}>
              {analysis.weaknesses.map((w, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2 rounded"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-mono font-bold" style={{ color: "#9a9a9a" }}>{w.point}</p>
                    {w.detail && <p className="text-[8px] font-mono mt-0.5" style={{ color: "#4a4a4a" }}>{w.detail}</p>}
                  </div>
                </div>
              ))}
            </Section>
          ...
          {analysis.session_analysis.length > 0 && (
            <Section title="セッション分析" color="#4a4a4a">
              {analysis.session_analysis.map((s, i) => (
                <div key={i} className="px-3 py-2 rounded"
                  style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.04)" }}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[8px] font-black tracking-widest" style={{ color: "#4a4a4a" }}>{s.session}</span>
                  </div>
                  <p className="text-[9px] font-mono leading-relaxed" style={{ color: "#9a9a9a" }}>{s.observation}</p>
                  {s.recommendation && (
          ...
          <Section title="リスク分析" color={AMBER}>
            <div className="px-3 py-3 rounded space-y-2"
              ...
              {([
                ...
                <div key={label}>
                  <p className="text-[7px] font-black tracking-widest mb-0.5" style={{ color: "#9a9a9a" }}>{label}</p>
                  <p className="text-[9px] font-mono leading-relaxed" style={{ color: "#9a9a9a" }}>{text}</p>
                </div>
              ))}
            </div>
          </Section>
            ...
            <Section title="推奨事項" color={CYAN}>
              {analysis.recommendations.map((r, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2 rounded"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-mono font-bold" style={{ color: "#9a9a9a" }}>{r.action}</p>
                    {r.rationale && (
                      <p className="text-[8px] font-mono mt-0.5" style={{ color: "#4a4a4a" }}>{r.rationale}</p>
                    )}
                  </div>
                </div>
              ))}
...
function ImprovementSection({ strategyId, analysisId, requiresMoreData }: {
  ...
  return (
    <div className="space-y-4 pt-3" style={{ borderTop: "1px solid rgba(0,0,0,0.04)" }}>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          style={{
            ...
            border:  `1px solid ${state === "generating" ? "rgba(0,0,0,0.06)" : "rgba(167,139,250,0.35)"}`,
            color:   state === "generating" ? "#9a9a9a" : PURPLE,
      ...
      {state === "rejected" && (
        <p className="text-[9px] font-mono" style={{ color: "#4a4a4a" }}>
      ...
      {improvement && state === "done" && (
        <div className="space-y-4">
                ...
                <div key={i} className="px-3 py-3 rounded"
                  ...
                  {c.type === "modify" && (
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-[8px]" style={{ color: "#9a9a9a" }}>→</span>
                  ...
                  {c.type === "add" && (
                    <p className="text-[8px] font-mono mb-1.5" style={{ color: "#9a9a9a" }}>
                      {JSON.stringify(c.to)}
                    </p>
                  )}
                  <p className="text-[8px] font-mono leading-relaxed" style={{ color: "#4a4a4a" }}>
                    {c.reason}
                  </p>
          ...
          <div className="px-3 py-3 rounded"
            style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.04)" }}>
            <p className="text-[8px] font-black tracking-[0.22em] mb-2" style={{ color: "#4a4a4a" }}>
              期待される効果（仮説）
            </p>
            <p className="text-[9px] font-mono leading-relaxed italic mb-2" style={{ color: "#9a9a9a" }}>
            ...
            {Object.entries(improvement.expected_effects.metric_targets).map(([k, v]) =>
              v ? (
                <div key={k} className="flex items-start gap-2">
                  ...
                  <span className="text-[8px] font-mono" style={{ color: "#4a4a4a" }}>{v}</span>
                </div>
              ) : null
            )}
          </div>
              ...
              <div className="space-y-1">
                {improvement.risks.map((r, i) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <p className="text-[8px] font-mono" style={{ color: "#4a4a4a" }}>{r}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Confidence */}
          <div className="flex items-center gap-3">
            <p className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>
              提案の信頼度
            </p>
            <div className="flex-1 h-1 rounded-full" style={{ background: "rgba(0,0,0,0.04)" }}>
              <div className="h-full rounded-full"
...
function CrossPhaseInterpretationSection({ strategyId }: { strategyId: string }) {
  ...
  return (
    <div className="mt-4 p-4 rounded" style={{ background: `${P4D_COLOR_r}0.04)`, border: `1px solid ${P4D_COLOR_r}0.15)` }}>
      ...
      <p className="text-[7px] font-mono mb-3" style={{ color: "#4a4a4a" }}>
        ...
        {ALL_PHASES.map(ph => {
          ...
          return (
            <span key={ph} className="text-[7px] font-black tracking-wider px-1.5 py-0.5 rounded"
              style={{
                background: isActive ? `${P4D_COLOR}15` : "rgba(0,0,0,0.03)",
                border:     `1px solid ${isActive ? P4D_COLOR + "40" : "rgba(0,0,0,0.04)"}`,
                color:      isActive ? P4D_COLOR : "#9a9a9a",
              }}>
              {PHASE_LABELS[ph]}
      ...
      {state === "done" && r && (
        <div className="mt-4 space-y-4">
          ...
          {r.overall_assessment && (
            <div>
              <p className="text-[8px] font-black tracking-widest mb-1" style={{ color: "#4a4a4a" }}>総合評価</p>
              <p className="text-[9px] font-mono leading-relaxed" style={{ color: "#9a9a9a" }}>{r.overall_assessment}</p>
            </div>
          )}
          ...
          {r.phase_observations && r.phase_observations.length > 0 && (
            <div>
              <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>フェーズ別観察</p>
              <div className="space-y-2">
                {r.phase_observations.map((obs, i) => (
                  <div key={i} className="p-2 rounded" style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)" }}>
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-[7px] font-black tracking-wider" style={{ color: P4D_COLOR }}>
                        {PHASE_LABELS[obs.phase] ?? obs.phase}
                      </span>
                    </div>
                    <p className="text-[8px] font-mono" style={{ color: "#9a9a9a" }}>{obs.observation}</p>
                    <p className="text-[7px] font-mono mt-0.5" style={{ color: "#4a4a4a" }}>{obs.supporting_data}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {r.cross_phase_synthesis && r.cross_phase_synthesis.length > 0 && (
            <div>
              <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>クロスフェーズ統合</p>
              <div className="space-y-2">
                {r.cross_phase_synthesis.map((syn, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="text-[7px] font-black tracking-widest mt-0.5 shrink-0"
                      style={{ color: SYNTH_COLORS[syn.type] ?? "#4a4a4a" }}>
                      {syn.type}
                    </span>
                    <p className="text-[8px] font-mono" style={{ color: "#9a9a9a" }}>{syn.observation}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {r.risk_dimensions && r.risk_dimensions.length > 0 && (
            <div>
              <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>リスク分析</p>
              <div className="space-y-1.5">
                {r.risk_dimensions.map((rd, i) => (
                  <div key={i} className="p-2 rounded" style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(0,0,0,0.03)" }}>
                    <p className="text-[7px] font-black tracking-widest mb-0.5" style={{ color: AMBER }}>
                      {RISK_LABELS[rd.dimension] ?? rd.dimension}
                    </p>
                    <p className="text-[8px] font-mono" style={{ color: "#9a9a9a" }}>{rd.assessment}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {r.limitations && r.limitations.length > 0 && (
            <div>
              <p className="text-[8px] font-black tracking-widest mb-1" style={{ color: "#4a4a4a" }}>制限事項</p>
              <div className="space-y-1">
                {r.limitations.map((lim, i) => (
                  <div key={i} className="flex items-start gap-1.5">
                    <span className="text-[7px] shrink-0 mt-0.5" style={{ color: "#9a9a9a" }}>•</span>
                    <p className="text-[7px] font-mono" style={{ color: "#4a4a4a" }}>{lim}</p>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="p-5 space-y-5 overflow-y-auto">
      ...
      {noParams && (
        <div className="flex items-center justify-center h-24">
          <p className="text-[9px] font-mono text-center" style={{ color: "#9a9a9a" }}>
      ...
      {!noParams && (
        <>
          {/* Parameter selection */}
          <div className="space-y-2">
            <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>
              パラメーター選択
            </p>
            {ranges.map((r, idx) => {
              ...
              return (
                <div key={r.field} className="rounded overflow-hidden"
                  style={{ border: `1px solid ${r.enabled ? `${GOLD_r}0.25)` : "rgba(0,0,0,0.04)"}`, background: r.enabled ? `${GOLD_r}0.03)` : "rgba(0,0,0,0.01)" }}>
                  {/* Toggle row */}
                  <div
                    className="flex items-center gap-3 px-3 py-2 cursor-pointer"
                    onClick={() => toggleRange(idx)}>
                    <div className="w-4 h-4 rounded shrink-0 flex items-center justify-center"
                      style={{ background: r.enabled ? GOLD : "rgba(0,0,0,0.04)", border: `1px solid ${r.enabled ? GOLD : "rgba(0,0,0,0.06)"}` }}>
                      {r.enabled && <span className="text-[8px] font-black" style={{ color: "#000" }}>✓</span>}
                    </div>
                    <span className="text-[8px] font-mono" style={{ color: r.enabled ? "#9a9a9a" : "#4a4a4a" }}>
                      {label}
                    </span>
                  </div>
                    ...
                    <div className="flex items-center gap-2 px-3 pb-2">
                      {(["min", "max", "step"] as const).map(key => (
                        <div key={key} className="flex items-center gap-1">
                          <span className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>{key.toUpperCase()}</span>
                          <input
                            type="number"
                            ...
                            style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.06)", color: "#9a9a9a" }}
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-[8px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>サンプル内比率</span>
              <input
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>組み合わせ数</span>
          ...
          <button
            ...
            style={{
              ...
              border:  `1px solid ${state === "running" ? "rgba(0,0,0,0.04)" : `${GOLD_r}0.35)`}`,
              color:   state === "running" ? "#9a9a9a" : GOLD,
        ...
        <div className="space-y-3">
          ...
          {jobData?.summary && (
            <div className="flex gap-4 px-3 py-2 rounded"
              style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.04)" }}>
              <div>
                <p className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>TOTAL</p>
                <p className="text-[10px] font-black font-mono" style={{ color: "#4a4a4a" }}>
                  {(jobData.total_combinations ?? 0).toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>安定ゾーン</p>
                <p className="text-[10px] font-black font-mono" style={{ color: GOLD }}>
                  {jobData.summary.stableZoneCount}
                </p>
              </div>
              <div>
                <p className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>ROBUST</p>
                <p className="text-[10px] font-black font-mono" style={{ color: NG }}>
                  {jobData.summary.robustCount}
                </p>
              </div>
              <div className="ml-auto text-right">
                <p className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>ISバー数</p>
                <p className="text-[9px] font-mono" style={{ color: "#4a4a4a" }}>
          ...
          {applyMsg && (
            <div className="px-3 py-2 rounded text-[9px] font-mono"
              style={{ background: "rgba(249,115,22,0.05)", border: "1px solid rgba(249,115,22,0.20)", color: NG }}>
              {applyMsg}
            </div>
          )}
            ...
            {candidates.map(c => {
              ...
              return (
                <div key={c.rank} className="rounded overflow-hidden"
                  style={{ background: c.adopted ? "rgba(249,115,22,0.04)" : "rgba(0,0,0,0.02)", border: `1px solid ${c.adopted ? "rgba(249,115,22,0.20)" : "rgba(0,0,0,0.04)"}` }}>
                  {/* Main row */}
                  <div className="grid items-center gap-2 px-3 py-2"
                    style={{ gridTemplateColumns: "40px 1fr 80px 80px 50px 70px" }}>
                    {/* Rank */}
                    <div className="flex items-center gap-1">
                      <span className="text-[11px] font-black font-mono" style={{ color: c.rank <= 3 ? GOLD : "#4a4a4a" }}>
                        #{c.rank}
                      </span>
                    </div>
                    ...
                    <div className="text-center">
                      <p className="text-[8px] font-mono font-bold" style={{ color: c.is_total_pips > 0 ? "#4a4a4a" : "#9a9a9a" }}>
                    ...
                    <div className="text-center">
                      ...
                      <div className="h-1 rounded-full mt-0.5" style={{ background: "rgba(0,0,0,0.04)" }}>
                    ...
                    <div className="text-right">
                      {c.adopted ? (
                        ...
                        <span className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>OOS不足</span>
                      ) : (
                        <button
                          ...
                          style={{
                            background: isApply ? "rgba(0,0,0,0.03)" : `${GOLD_r}0.10)`,
                            border:     `1px solid ${isApply ? "rgba(0,0,0,0.04)" : `${GOLD_r}0.30)`}`,
                            color:      isApply ? "#9a9a9a" : GOLD,
    ...
    <div className="pt-5 space-y-4" style={{ borderTop: `1px solid ${WF_BLUE_r}0.15)` }}>
      ...
      <div className="flex items-center gap-4 flex-wrap">
        {([
          ...
          <div key={label} className="flex items-center gap-1.5">
            <span className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>{label.toUpperCase()}</span>
            <input
              ...
              style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.06)", color: "#9a9a9a" }}
            />
            <span className="text-[7px] font-mono" style={{ color: "#9a9a9a" }}>M</span>
          </div>
        ))}
      </div>
      ...
      <button
        ...
        style={{
          ...
          border:  `1px solid ${state === "running" ? "rgba(0,0,0,0.04)" : `${WF_BLUE_r}0.35)`}`,
          color:   state === "running" ? "#9a9a9a" : WF_BLUE,
          cursor:  state === "running" ? "not-allowed" : "pointer",
        }}>
      ...
      {state === "done" && result && (
        <div className="space-y-3">
          ...
          <div className="px-4 py-3 rounded"
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[7px] font-mono tracking-widest mb-1" style={{ color: "#9a9a9a" }}>判定</p>
                <p className="text-lg font-black tracking-widest" style={{ color: verdictColorWF(result.verdict) }}>
                  {result.verdict}
                </p>
              </div>
              <div className="text-right">
                <p className="text-[7px] font-mono tracking-widest mb-1" style={{ color: "#9a9a9a" }}>一貫性スコア</p>
                <p className="text-base font-black font-mono" style={{ color: result.consistencyScore === null ? "#9a9a9a" : result.consistencyScore >= 0.7 ? NG : result.consistencyScore >= 0.5 ? AMBER : RED }}>
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {[
              ...
              { label: "スキップ",    val: String(result.skippedWindowCount),
                color: result.skippedWindowCount > 0 ? AMBER : "#9a9a9a" },
            ].map(({ label, val, color }) => (
              <div key={label} className="px-2 py-2 rounded"
                style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)" }}>
                <p className="text-[7px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>{label}</p>
                <p className="text-[11px] font-black font-mono mt-0.5" style={{ color: color ?? "#9a9a9a" }}>{val}</p>
              </div>
            ))}
          </div>
                ...
                {Object.entries(result.parameterStability).map(([field, stab]) => {
                  ...
                  return (
                    <div key={field} className="flex items-center gap-2">
                      <span className="text-[7px] font-mono w-24 shrink-0" style={{ color: "#4a4a4a" }}>{short}</span>
                      <div className="flex-1 h-1 rounded-full" style={{ background: "rgba(0,0,0,0.04)" }}>
                        <div className="h-full rounded-full" style={{ width: `${stab*100}%`, background: stabCol }} />
                      </div>
          ...
          {result.recommendedParams && (
            <div className="px-3 py-3 rounded"
              ...
              <p className="text-[7px] font-mono mb-2" style={{ color: "#4a4a4a" }}>
              ...
              <div className="space-y-1">
                {Object.entries(result.recommendedParams).map(([field, val]) => {
                  ...
                  return (
                    <div key={field} className="flex items-center gap-2">
                      <span className="text-[7px] font-mono" style={{ color: "#4a4a4a" }}>{short}</span>
              ...
              {showWindows && (
                <div className="mt-2 space-y-1.5">
                  {result.windows.map((w, i) => {
                    ...
                    const col    = w.skipped ? "#9a9a9a" : passed ? NG : RED;
                    ...
                    return (
                      <div key={i} className="px-3 py-2 rounded"
                        ...
                        {!w.skipped && (
                          <div className="flex gap-3 mt-1 text-[7px] font-mono" style={{ color: "#4a4a4a" }}>
                            <span>{params}</span>
    ...
    <div className="mt-4 p-4 rounded" style={{ background: `${MC_VIOLET_r}0.04)`, border: `1px solid ${MC_VIOLET_r}0.15)` }}>
      ...
      <div className="flex flex-wrap gap-3 mb-3">
        <div className="flex flex-col gap-1">
          <span className="text-[7px] font-mono" style={{ color: "#4a4a4a" }}>試行回数</span>
          <input
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[7px] font-mono" style={{ color: "#4a4a4a" }}>DD閾値 (%)</span>
          <input
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[7px] font-mono" style={{ color: "#4a4a4a" }}>シード（任意）</span>
          <input
      ...
      {state === "done" && result && (
        <div className="mt-4 space-y-4">
          ...
          {/* Original Backtest */}
          <div>
            <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>元のバックテスト</p>
                ...
                <thead>
                  <tr>
                    {["", "P5", "P25", "P50", "P75", "P95"].map(h => (
                      <th key={h} className="text-[7px] font-black text-right pr-3 pb-1" style={{ color: "#4a4a4a" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  ...
                  {pset && (
                    <tr>
                      <td className="text-[7px] font-mono pr-3 py-1" style={{ color: "#4a4a4a" }}>合計PIPS</td>
                      {([pset.p5, pset.p25, pset.p50, pset.p75, pset.p95] as number[]).map((v, i) => {
                        const col = v < 0 ? RED : v > 0 ? NG : "#9a9a9a";
                        return (
                  ...
                  {ddset && (
                    <tr>
                      <td className="text-[7px] font-mono pr-3 py-1" style={{ color: "#4a4a4a" }}>最大DD %</td>
                      {([ddset.p5, ddset.p25, ddset.p50, ddset.p75, ddset.p95] as number[]).map((v, i) => {
                        const thresh = result.drawdownThresholdPct;
                        const col = v >= thresh ? RED : v >= thresh * 0.7 ? AMBER : "#9a9a9a";
                        return (
                          <td key={i} className="text-right pr-3 py-1">
                            <span className="text-[9px] font-black font-mono" style={{ color: col }}>
                              {v.toFixed(1)}%
          ...
          {/* Key Probabilities */}
          <div>
            <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>主要確率</p>
            <div className="flex flex-wrap gap-3">
              <div className="p-2 rounded flex-1" style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)", minWidth: 140 }}>
                <p className="text-[7px] font-mono mb-0.5" style={{ color: "#4a4a4a" }}>損失確率（合計PIPS&lt;0）</p>
              ...
              <div className="p-2 rounded flex-1" style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.05)", minWidth: 140 }}>
                <p className="text-[7px] font-mono mb-0.5" style={{ color: "#4a4a4a" }}>DD超過確率（DD≥{result.drawdownThresholdPct}%）</p>
          ...
          {/* Original Position */}
          <div>
            <p className="text-[8px] font-black tracking-widest mb-2" style={{ color: "#4a4a4a" }}>元の順序の位置</p>
            <div className="p-2 rounded inline-flex items-center gap-2" style={{ background: `${MC_VIOLET_r}0.08)`, border: `1px solid ${MC_VIOLET_r}0.2)` }}>
              <span className="text-[9px] font-mono" style={{ color: "#9a9a9a" }}>元の順序:</span>
              <span className="text-[13px] font-black font-mono" style={{ color: MC_VIOLET }}>
                P{Math.round(result.originalPercentileRank)}
              </span>
              <span className="text-[7px] font-mono" style={{ color: "#4a4a4a" }}>
...
function VersionsTab({ strategyId, onApplyDone }: {
  ...
  if (loading) {
    return (
      <div className="p-5 flex items-center justify-center h-48">
        <p className="text-[9px] font-mono tracking-widest" style={{ color: "#9a9a9a" }}>◌ バージョン読み込み中...</p>
      </div>
    );
  }
        ...
        return (
          <div key={v.id}>
            ...
            <div
              ...
              style={{
                background: isSelected ? "rgba(167,139,250,0.06)" : "rgba(0,0,0,0.02)",
                border: `1px solid ${isSelected ? "rgba(167,139,250,0.30)" : "rgba(0,0,0,0.04)"}`,
              }}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className="text-[7px] font-mono px-1.5 py-0.5 rounded"
                    style={{ background: "rgba(0,0,0,0.05)", color: "#4a4a4a" }}>
              ...
              {v.change_summary && (
                <p className="text-[8px] font-mono mt-1.5 leading-relaxed" style={{ color: "#4a4a4a" }}>
                  {v.change_summary}
                </p>
              )}

              {v.backtest && (
                <div className="flex gap-3 mt-1.5 text-[7px] font-mono" style={{ color: "#9a9a9a" }}>
            ...
            {isSelected && (
              <div className="mt-1 mx-1 px-4 py-4 rounded space-y-4"
                ...
                {detailLoading && (
                  <p className="text-[9px] font-mono" style={{ color: "#9a9a9a" }}>◌ Loading...</p>
                )}
                ...
                {comparison && !detailLoading && (
                  <div>
                    ...
                    <div className="px-3 py-2 rounded mb-2"
                      ...
                      <p className="text-[8px] font-mono mt-1" style={{ color: "#4a4a4a" }}>
                        {comparison.summary}
                      </p>
                    </div>
                      ...
                      <div className="space-y-0.5 mb-1">
                        {comparison.improvements.map((imp, i) => (
                          <div key={i} className="flex items-center gap-2 text-[8px] font-mono">
                            ...
                            <span style={{ color: "#4a4a4a" }}>{imp.metric}</span>
                            <span style={{ color: "#9a9a9a" }}>{String(imp.before)} → {String(imp.after)}</span>
                          </div>
                        ))}
                      </div>
                    )}
                      ...
                      <div className="space-y-0.5 mb-1">
                        {comparison.regressions.map((reg, i) => (
                          <div key={i} className="flex items-center gap-2 text-[8px] font-mono">
                            ...
                            <span style={{ color: "#4a4a4a" }}>{reg.metric}</span>
                            <span style={{ color: "#9a9a9a" }}>{String(reg.before)} → {String(reg.after)}</span>
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {detail && !detailLoading && (
                  <div>
                    ...
                    <div className="text-[7px] font-mono space-y-0.5" style={{ color: "#9a9a9a" }}>
                      {(() => {
                ...
                {!v.isActive && (
                  <button
                    ...
                    style={{
                      background: restoring ? "rgba(0,0,0,0.03)" : "rgba(100,116,139,0.10)",
                      border:     "1px solid rgba(100,116,139,0.25)",
                      color:      restoring ? "#9a9a9a" : "#4a4a4a",
...
export function StrategyDetailModal({
  ...
  const col = typeColor[strategy.strategy_type] ?? "#4a4a4a";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Tabs */}
        <div className="flex gap-0 px-5 shrink-0" style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
