...
function getDataQuality(ind: IndicatorData): { score: number; label: string; color: string } {
  ...
  if (tfCount >= 4 && fresh) return { score: 90, label: "COMPLETE",   color: "#f97316" };
  if (tfCount >= 3 && fresh) return { score: 75, label: "GOOD",       color: "#f97316" };
...
function statusColor(s: DataStatus): string {
  switch (s) {
    case 'LIVE':               return '#f97316';
    case 'FRESH':              return '#f97316';
...
const TFBar = memo(function TFBar({
  ...
  if (ema21 === 0) return (
    <div className="flex items-center gap-2 h-6">
      ...
      <div className="flex-1 h-2 bg-[rgba(0,0,0,0.04)] rounded-sm flex items-center px-1">
  ...
  const bull = dir === "BULL";
  const bear = dir === "BEAR";
  const col  = bull ? "#f97316" : bear ? "#ff1a4e" : "#374151";
  ...
  return (
    <div className="flex items-center gap-2 h-6">
      ...
      {/* Direction bar */}
      <div className="relative flex-1 h-2.5 bg-[rgba(0,0,0,0.04)] rounded-sm overflow-hidden">
        <div className="absolute inset-y-0 left-0 rounded-sm"
          style={{
            width: `${pct}%`,
            background: bull
              ? "linear-gradient(90deg,rgba(249,115,22,0.15),#f97316)"
              : bear
...
const SymbolCard = memo(function SymbolCard({
  ...
  const biasColor = bias === "BUY" ? "#f97316" : bias === "SELL" ? "#ff1a4e" : "#f97316";
  const dirColor  = overallDir === "BULL" ? "#f97316" : overallDir === "BEAR" ? "#ff1a4e" : "#1e3a5f";

  const borderColor = isScanning ? "#f97316"
    : overallDir === "BULL" ? "#f9731650"
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: isScanning
          ? "0 0 24px rgba(249,115,22,0.15), inset 0 0 40px rgba(249,115,22,0.03)"
          : overallDir === "BULL" ? "0 0 12px rgba(249,115,22,0.08)"
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-0.5 pointer-events-none"
        style={{
          background: isScanning
            ? "linear-gradient(90deg,transparent 0%,#f97316 40%,#f97316 60%,transparent 100%)"
      ...
      {(["tl","tr","bl","br"] as const).map((pos, i) => {
        const bCol = isScanning ? "#f97316" : borderColor;
        return (
      ...
      <div className="absolute inset-0 pointer-events-none z-20 transition-opacity duration-500"
        style={{ background: "rgba(249,115,22,0.06)", opacity: tickFlash ? 1 : 0 }}/>
      ...
      <div className="relative z-[5] flex flex-col flex-1 p-3 gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              className={cn("w-1.5 h-1.5 rounded-full shrink-0 transition-colors duration-300",
                isLive ? "bg-green-400" : ageMs < 30000 ? "bg-yellow-500" : "bg-[#f0efec]"
              )}
        ...
        <div className="shrink-0 flex items-center justify-center py-2 border"
          ...
          <div className="flex items-center gap-2">
            {bias === "BUY"  && <TrendingUp  size={18} style={{ color: "#f97316" }}/>}
            {bias === "SELL" && <TrendingDown size={18} style={{ color: "#ff1a4e" }}/>}
            {bias === "WAIT" && <Minus size={18} style={{ color: "#f97316" }}/>}
                ...
                <span
                  ...
                  style={{
                    ...
                    textShadow: priceGlow
                      ...
                      : overallDir === "BULL" ? "0 0 12px rgba(249,115,22,0.15)"
        ...
        {/* Row 5: COT */}
        <div className="shrink-0 border-t border-[rgba(0,0,0,0.06)] pt-2">
          <div className="flex items-center justify-between text-[7.5px] font-mono">
            <div className="flex items-center gap-1.5">
              <ShieldAlert size={7} style={{ color: "#f9731680" }}/>
              <span className="font-bold" style={{ color: "#6b7280" }}>COT</span>
              {intelligenceLoading && (
                <span className="text-[5.5px] font-mono" style={{ color: "#f9731660", animation: "avl-blink 1s ease-in-out infinite" }}>…</span>
              )}
            </div>
            {!intelligence || intelligence.cot.status === 'NO_DATA' || intelligence.cot.status === 'SOURCE_UNAVAILABLE' ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold tabular-nums"
                  style={{ color: (intelligence.cot.netPct ?? 0) > 0 ? '#f97316' : (intelligence.cot.netPct ?? 0) < 0 ? '#ff1a4e' : '#6b7280' }}>
        ...
        <div className="shrink-0 pt-2 border-t border-[rgba(0,0,0,0.06)]">
          ...
          {/* DQ progress bar */}
          <div className="h-1 bg-[rgba(0,0,0,0.04)] rounded-full overflow-hidden">
...
function AICommandBar({
  ...
  return (
    <div className="shrink-0 border-b border-[rgba(0,0,0,0.06)] bg-white relative overflow-hidden">
      {/* Animated top line */}
                ...
                <div key={stage} className="flex items-center gap-1 min-w-0 flex-1">
                  <div className="flex flex-col items-center min-w-0 flex-1">
                    <div
                      ...
                      style={{
                        background: isCurrent
                          ...
                          ? "#f9731650"
                          : "rgba(0,0,0,0.06)",
                        boxShadow: isCurrent ? "0 0 6px rgba(249,115,22,0.8)" : "none",
                    ...
                    <span
                      ...
                      style={{
                        color: isCurrent ? "#f97316"
                          : isDone ? "#f9731660"
                          : "#1f2a36",
                        textShadow: isCurrent ? "0 0 6px rgba(249,115,22,0.6)" : "none",
                      }}>
                      {stage}
                    </span>
                  </div>
                  {i < STAGES.length - 1 && (
                    <span className="text-[6px] shrink-0"
                      style={{ color: isDone ? "#f9731640" : "rgba(0,0,0,0.06)" }}>›</span>
                  )}
                </div>
              );
            })}
            ...
            return (
              <div key={sym} className="flex flex-col items-center gap-0.5">
                <div className="h-0.5 w-8 rounded-full transition-all duration-300"
                  style={{
                    background: active ? "#f97316" : done ? "#f9731650" : "rgba(0,0,0,0.06)",
                    boxShadow: active ? "0 0 4px rgba(249,115,22,0.6)" : "none",
                  }}/>
                <span className="text-[5.5px] font-mono"
                  style={{ color: active ? "#f97316" : done ? "#374151" : "#1f2a36" }}>
                  {sym}
                </span>
              </div>
            );
...
function AIReasoningFlow({
  ...
  return (
    <div className="shrink-0 border-t border-[rgba(0,0,0,0.06)] bg-white relative overflow-hidden">
      ...
      <div className="px-4 py-2.5 flex items-center gap-4">
        ...
        <div className="shrink-0">
          ...
          <p className="text-[8.5px] font-mono font-bold leading-none"
            style={{ color: isDone ? "#f97316" : "#f97316" }}
            suppressHydrationWarning>
            ...
            return (
              <div key={stage} className="flex items-center gap-1 shrink-0">
                {i > 0 && <div className="w-3 h-px"
                  style={{ background: isDone_ || isCurrent_ ? "#f9731620" : "rgba(0,0,0,0.06)" }}/>}
                <div className="flex flex-col items-center gap-0.5">
                  <div
                    ...
                    style={{
                      ...
                      background: isCurrent_ ? "#f97316"
                        : isDone_ ? "#f97316"
                        : "rgba(0,0,0,0.06)",
                      boxShadow: isCurrent_ ? "0 0 8px rgba(249,115,22,0.8)" : "none",
                      animation: isCurrent_ ? "avl-blink 0.8s ease-in-out infinite" : "none",
                    }}/>
                  <span className="text-[5px] font-mono font-bold"
                    style={{
                      color: isCurrent_ ? "#f97316" : isDone_ ? "#f9731670" : "#1f2a36",
                    }}>
                    {stage}
                  </span>
                </div>
        ...
        <div className="shrink-0 text-right">
          ...
          <p className="text-[10px] font-black font-mono"
            style={{ color: "#f97316", textShadow: "0 0 6px rgba(249,115,22,0.4)" }}
            suppressHydrationWarning>
            {currentSym}
          </p>
        </div>
      ...
      <div className="pl-14 pr-4 md:px-5 py-2.5 flex items-center gap-3 md:gap-5">
        ...
        <div className="shrink-0 flex items-center gap-2.5">
          <div className="flex gap-0.5">
            <div className="w-0.5 h-7" style={{ background: "#f97316", boxShadow: "0 0 8px #f97316" }}/>
          ...
          <div>
            ...
            <p className="text-[12px] font-black font-mono tracking-[0.12em] leading-none"
              style={{ color: "#f97316", textShadow: "0 0 14px rgba(249,115,22,0.45)" }}>
            ...
            <div key={label} className="flex items-center gap-1">
              <div
                className={cn("w-1.5 h-1.5 rounded-full transition-colors duration-500",
                  isConnected ? "bg-green-400" : "bg-[#f0efec]")}
                style={isConnected && tickFlash
                  ? { animation: "avl-blink 0.3s ease-in-out 3", boxShadow: "0 0 6px #f97316" }
                  : isConnected
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="text-center">
            <p className={cn("text-[20px] font-black font-mono tabular-nums leading-none",
              ...
              style={liveCount > 0 ? { textShadow: "0 0 10px rgba(249,115,22,0.4)" } : undefined}>
              {liveCount}
            </p>
          ...
          <div className="flex items-center gap-1.5">
            <div
              className={cn("w-2 h-2 rounded-full transition-colors duration-500",
                isConnected ? "bg-green-400" : "bg-[#f0efec]")}
              style={isConnected ? { boxShadow: "0 0 6px rgba(249,115,22,0.5)" } : undefined}/>
...
function ExpandedPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.5)] backdrop-blur-sm p-4"
      ...
      <div className="w-full max-w-xl border border-[rgba(0,0,0,0.08)] bg-[#f8f7f4] overflow-hidden relative"
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto avl-scroll">
            ...
            <div className="space-y-2">
              {TFS.map(tf => {
                ...
                if (!d || d.ema21 === 0) return (
                  <div key={tf} className="flex items-center gap-3 border border-[rgba(0,0,0,0.06)] px-3 py-2">
                ...
                const col_  = bull_ ? "#f97316" : bear_ ? "#ff1a4e" : "#374151";
                return (
                  <div key={tf} className="border px-3 py-2"
                    style={{
                      borderColor: bull_ ? "#f9731620" : bear_ ? "#ff1a4e20" : "rgba(0,0,0,0.06)",
                      background: bull_ ? "rgba(249,115,22,0.02)" : bear_ ? "rgba(255,26,78,0.02)" : "transparent",
                    }}>
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        <span className="text-[8px] font-mono font-bold w-7" style={{ color: col_ }}>{tf}</span>
                        {bull_ && <TrendingUp size={9} style={{ color: "#f97316" }}/>}
                    ...
                    <div className="h-1.5 bg-[rgba(0,0,0,0.04)] rounded-full overflow-hidden mb-1.5">
          ...
          <div className="border border-cyan-900/30 bg-[#020810] p-3 space-y-3">
            ...
            {intelligence ? (
              <div className="space-y-2">
                {/* COT */}
                <div className="border border-[rgba(0,0,0,0.06)] px-3 py-2">
                  <div className="flex items-center justify-between mb-1.5">
                    <div>
                  ...
                  {intelligence.cot.longPct !== null ? (
                    <>
                      <div className="flex items-center gap-4 text-[7.5px] font-mono mb-1">
                        ...
                        <span className="font-bold tabular-nums"
                          style={{ color: (intelligence.cot.netPct ?? 0) > 0 ? '#f97316' : (intelligence.cot.netPct ?? 0) < 0 ? '#ff1a4e' : '#374151' }}>
                          NET {(intelligence.cot.netPct ?? 0) > 0 ? '+' : ''}{intelligence.cot.netPct ?? 0}%
                        </span>
                      </div>
                      <div className="h-1 bg-[rgba(0,0,0,0.04)] rounded-full overflow-hidden mb-1.5">
                        <div className="h-full rounded-full"
                          style={{ width: `${intelligence.cot.longPct}%`, background: 'linear-gradient(90deg,rgba(249,115,22,0.3),#f97316)' }}/>
                      </div>
                      {/* 枚数詳細 */}
                      <div className="flex gap-4 text-[7px] font-mono text-gray-600 mb-1">
                        ...
                        <span>NET <span className="tabular-nums" style={{ color: (intelligence.cot.netContracts ?? 0) >= 0 ? '#f9731680' : '#ff1a4e80' }}>
