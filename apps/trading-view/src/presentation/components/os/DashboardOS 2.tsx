...
import { AICore3D }            from "@/presentation/components/os/AICore3D";
...
export function DashboardOS() {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      client.onIndicators((ind) => {
        setIndicators(ind);
        setAgentStatus("market", "active", `${ind.symbol} EMA/ATR 更新`);
  ...
  // =================================================================
  return (
    <div className="flex flex-col h-full bg-[#05070A] overflow-hidden relative">
      {/* スキャンライン背景 */}
      <div className="absolute inset-0 avl-grid-bg opacity-[0.035] pointer-events-none z-0"/>
      ...
      <div className="relative z-10 h-11 shrink-0 flex items-center px-5 gap-6 bg-[#05070A] border-b border-cyan-900/25">
        ...
        {/* Time */}
        <div className="flex items-center gap-2 text-[10px] font-mono" suppressHydrationWarning>
          <span className="text-gray-100 tabular-nums font-semibold">{time.toLocaleTimeString("ja-JP",{hour:"2-digit",minute:"2-digit",second:"2-digit"})}</span>
          <span className="text-gray-600">JST</span>
          <span className="text-gray-500">{time.toLocaleDateString("en-US",{year:"numeric",month:"2-digit",day:"2-digit"})}</span>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden z-10 relative">
        ...
        {/* ╔══════════════════════════════╗
            ║  LEFT PANEL                  ║
            ╚══════════════════════════════╝ */}
        <div className="w-[262px] shrink-0 flex flex-col border-r border-cyan-900/20 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-3 space-y-3">

            {/* Account Overview */}
            <div className="border border-cyan-900/25 bg-[#070a10] relative overflow-hidden">
              <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-cyan-500/30 to-transparent"/>
            ...
            <div className="border border-cyan-900/25 bg-[#070a10] relative overflow-hidden">
              ...
              <div className="px-3 py-2.5">
                <p className="text-[8px] text-cyan-400/60 font-mono tracking-[0.2em] mb-2 flex items-center gap-1.5">
                  <BarChart2 size={8}/> MARKET WATCH
                </p>
                <div className="space-y-1">
                  {watchlist.map(w => {
                    const ind = indicators[w.symbol.replace("/","").toUpperCase()];
                    const h4  = ind?.timeframes?.H4;
                    const up  = h4 ? h4.ema21 > h4.ema200 : null;
                    return (
                      <div key={w.symbol} className="flex items-center gap-2 py-0.5 group hover:bg-cyan-950/15 px-1 transition-colors">
                        <div className={cn("shrink-0", up === true ? "text-green-400" : up === false ? "text-red-400" : "text-gray-700")}>
                          {up === true ? <TrendingUp size={9}/> : up === false ? <TrendingDown size={9}/> : <Minus size={9}/>}
                        </div>
                        <span className={cn("text-[9px] font-mono font-bold flex-1", w.bid>0?"text-cyan-300":"text-gray-700")}>{w.symbol}</span>
                        <span className="text-[9px] font-mono tabular-nums text-gray-100">{w.bid>0?w.bid.toFixed(w.bid>100?3:5):"—"}</span>
                        {w.bid>0 && <span className={cn("text-[7px] font-mono tabular-nums", w.spread>5?"text-red-400":"text-gray-600")}>{w.spread.toFixed(1)}p</span>}
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          {/* AI Core 3D Sphere */}
          <div className="flex-1 relative flex flex-col items-center justify-start overflow-hidden">

            {/* Sphere */}
            <div className="relative flex items-center justify-center flex-1 w-full">
              <AICore3D mode={mode} isActive={isActive} isThinking={thinking} voiceStatus={voice.status} width={520} height={520}/>

              {/* AVL AI text overlay */}
              <div className="absolute bottom-[68px] left-0 right-0 text-center pointer-events-none">
                <p className="font-black font-mono tracking-[0.5em]"
                  style={{fontSize:30, color:neonHex, textShadow:`0 0 20px ${neonHex}, 0 0 40px ${neonHex}88, 0 0 80px ${neonHex}44`}}>
                  AVL AI
                </p>
                <p className="font-mono tracking-[0.3em] mt-1"
                  style={{fontSize:9, color:`${neonHex}cc`, textShadow:`0 0 8px ${neonHex}`}}>
                  {modeLabel2}
                </p>
              </div>
              ...
              {/* HUD corner brackets */}
              {[
                "absolute top-4 left-4 w-5 h-5 border-t-2 border-l-2",
                "absolute top-4 right-4 w-5 h-5 border-t-2 border-r-2",
                "absolute bottom-4 left-4 w-5 h-5 border-b-2 border-l-2",
                "absolute bottom-4 right-4 w-5 h-5 border-b-2 border-r-2",
              ].map((cls,i) => (
                <div key={i} className={cn(cls, "pointer-events-none")}
                  style={{borderColor:`${neonHex}50`, boxShadow:`0 0 6px ${neonHex}30`}}/>
              ))}

              {/* Signal notification */}
              {monitor.signals.length > 0 && (
                <div className="absolute top-3 left-0 right-0 flex justify-center pointer-events-none">
                  <div className="flex items-center gap-2 px-3 py-1.5 border" style={{borderColor:`${neonHex}40`, backgroundColor:`${neonHex}10`}}>
                    ...
                    <span className="text-[7px] font-mono border px-1" style={{borderColor:`${neonHex}40`,color:`${neonHex}88`}}>
                      +{monitor.signals.length}
                    </span>
                  </div>
                </div>
              )}

              {/* Chat overlay (half-height) */}
              {showChat && (
                <div className="absolute inset-x-0 bottom-0 h-[55%] flex flex-col"
                  style={{background:`linear-gradient(to bottom, transparent, #05070Acc 20%, #05070Aee 50%)`}}>
                  <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 pt-8 pb-2 space-y-2">
                    {messages.filter(m=>m.role!=="system").map(msg => (
                      <div key={msg.id} className={cn("flex", msg.role==="user"?"justify-end":"justify-start")}>
                        {msg.role==="assistant" && (
                          <div className="w-4 h-4 border border-cyan-500/30 flex items-center justify-center mr-1.5 mt-0.5 shrink-0">
                            <ChevronRight size={8} className="text-cyan-400"/>
                          </div>
                        )}
                        <div className={cn("max-w-[85%] px-3 py-2 border text-[10px] font-mono",
                          msg.role==="user" ? "border-blue-800/40 bg-blue-950/30 text-blue-100" : "border-cyan-900/30 bg-[#070a10]/80"
                        )}>
                    ...
                    {thinking && (
                      <div className="flex items-center gap-2 pl-6">
                        ...
                        <span className="text-[8px] text-cyan-500/50 font-mono">AVL AI 分析中...</span>
                      </div>
                    )}
                  </div>
                  {orderProposal && (
            ...
            {/* AI Status Bar */}
            <div className="shrink-0 w-full border-t border-cyan-900/20 px-5 py-3 bg-[#05070A]">
              <div className="flex items-center gap-5">
                <div>
                  <p className="text-[7px] text-gray-700 font-mono tracking-[0.2em] mb-1">AI STATUS</p>
                  {/* Voice waveform */}
                  <div className="flex items-end gap-0.5 h-5">
                    {[3,5,8,12,16,20,16,12,8,5,3,6,10,15,10,6].map((h,i) => (
                      <div key={i} className="w-0.5 rounded-full"
                        style={{
                          height: isActive ? h*1.3 : h*0.5,
                          backgroundColor: neonHex,
                          opacity: isActive ? 0.7 : 0.2,
                          boxShadow: isActive ? `0 0 3px ${neonHex}` : "none",
                          animation: isActive
                            ? `avl-wave-bar ${0.6+i*0.06}s ease-in-out ${i*0.06}s infinite alternate`
                            : "none",
                          transformOrigin: "bottom",
                        }}
                      />
                ...
                <p className="text-[13px] font-mono tracking-[0.2em] font-semibold"
                  style={{color:neonHex, textShadow:`0 0 10px ${neonHex}88`}}>
                  {aiStatusText}
                </p>
                <div className="ml-auto flex items-center gap-2">
                  <button onClick={()=>setShowChat(v=>!v)}
                    className="text-[7.5px] font-mono text-gray-700 hover:text-cyan-400 transition-colors tracking-wider border border-gray-800/40 hover:border-cyan-800/40 px-2 py-1">
                    {showChat?"◀ SPHERE":"◀ CHAT"}
                  </button>
          ...
          {/* Quick Commands + Voice Mic */}
          <div className="shrink-0 border-t border-cyan-900/15 px-4 py-2.5 bg-[#05070A] flex items-center gap-3">
            {/* Mic button */}
            <button type="button" disabled={!isConnected}
              onClick={() => voice.status==="idle" ? voice.start(activeSymbol) : voice.stop()}
              className={cn("flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all shrink-0",
                isVoiceActive
                  ? "border-cyan-400/70 bg-cyan-950/40 text-cyan-300"
                  : "border-gray-700/50 text-gray-600 hover:border-cyan-700/50 hover:text-cyan-400 hover:bg-cyan-950/20",
                "disabled:opacity-30 disabled:cursor-not-allowed"
              )}
              style={isVoiceActive ? {boxShadow:`0 0 16px ${neonHex}50`} : undefined}>
              {isVoiceActive ? <Mic size={16} className="animate-pulse"/> : <MicOff size={16}/>}
            </button>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {QUICK_CMDS.map(({label,cmd}) => (
                <button key={label} onClick={()=>sendMessage(cmd)} disabled={thinking||!isConnected}
                  className="flex items-center gap-1 px-3 py-1.5 text-[8px] font-mono border border-cyan-900/25 text-gray-600 hover:border-cyan-700/40 hover:text-cyan-400 hover:bg-cyan-950/15 disabled:opacity-20 transition-all tracking-wide">
                  <Zap size={8}/>{label}
                </button>
              ))}
            </div>
            ...
            {voice.status !== "idle" && (
              <div className="ml-auto flex items-center gap-3">
                <div className="flex items-end gap-0.5">
                  {[3,6,10,15,10,6,3].map((h,i) => (
                    <div key={i} className="w-0.5 rounded-full" style={{height:h,backgroundColor:neonHex,opacity:0.7,animation:`avl-wave-bar 0.7s ease-in-out infinite alternate`,animationDelay:`${i*0.1}s`}}/>
                  ))}
                </div>
                <span className="text-[8px] font-mono tracking-wider" style={{color:neonHex}}>VOICE · {voice.status.toUpperCase()}</span>
                {voice.muted && <span className="text-[7px] text-yellow-400 font-mono">MUTED</span>}
                <button onClick={voice.interrupt} className="text-[7px] text-gray-600 hover:text-red-400 font-mono transition-colors">INTERRUPT</button>
              </div>
            )}
          </div>
        </div>
        ...
        {/* ╔══════════════════════════════╗
            ║  RIGHT PANEL                  ║
            ╚══════════════════════════════╝ */}
        <div className="w-[276px] shrink-0 flex flex-col border-l border-cyan-900/20 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
      ...
      ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
      <div className="relative shrink-0 border-t border-cyan-900/25 bg-[#05070A] px-5 py-3 z-10">
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent"/>
        <form onSubmit={handleSubmit} className="flex items-center gap-3">
          <textarea ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleKey}
            placeholder={isConnected ? "AVL AI へのコマンドを入力... (Enter送信 | Shift+Enterで改行)" : "MT5 未接続"}
            disabled={thinking||!isConnected} rows={1}
            className={cn(
              "flex-1 resize-none bg-transparent border border-cyan-900/30 text-gray-200 text-[11px] px-4 py-2.5 font-mono",
              "placeholder:text-gray-700 focus:outline-none focus:border-cyan-700/50 transition-colors",
              "disabled:opacity-40 disabled:cursor-not-allowed",
              "min-h-[40px] max-h-[80px]"
            )}
            style={{height:"40px"}}
            onInput={e=>{const t=e.currentTarget;t.style.height="40px";t.style.height=Math.min(t.scrollHeight,80)+"px";}}
          />
          {/* Waveform or status */}
          <div className="flex items-end gap-0.5 h-5 shrink-0">
            {[2,4,6,4,6,4,2,4,6,4,2].map((h,i) => (
              <div key={i} className="w-0.5 rounded-full"
                style={{
                  height: isVoiceActive ? h*2 : h,
                  backgroundColor: "#00e5ff",
                  opacity: isVoiceActive ? 0.7 : 0.15,
                  animation: isVoiceActive
                    ? `avl-wave-bar 0.8s ease-in-out ${i*0.07}s infinite alternate`
                    : "none",
                }}/>
            ))}
          </div>
          <span className="text-[8px] text-gray-700 font-mono shrink-0">
            {isVoiceActive ? "VOICE ACTIVE" : "またはマイクに話しかけてください"}
          </span>
          <button type="submit" disabled={!input.trim()||thinking||!isConnected}
            className={cn("flex items-center justify-center w-10 h-10 border transition-all shrink-0",
              "border-cyan-700/40 text-cyan-400 hover:bg-cyan-950/30 hover:border-cyan-600/60",
