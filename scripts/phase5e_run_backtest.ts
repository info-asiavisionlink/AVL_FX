/**
 * Phase 5-E Backtest Script
 *
 * Runs TWO strategies (v4 LONG + v4 SHORT) against EURUSD bar data.
 *
 * Hypothesis: RSI(14) used as a STATE filter (not crossover):
 *   LONG:  RSI(14) M5 BELOW 60  (exclude overbought entries)
 *   SHORT: RSI(14) M5 ABOVE 40  (exclude oversold entries)
 *
 * Baselines:
 *   Phase 5-C: 2,290 total trades (v2 LONG + v2 SHORT)
 *   Phase 5-D: 1,106 total trades (v3 LONG + v3 SHORT, 51.7% reduction)
 *
 * Strategy IDs are set after running phase5e_create_strategy.ts.
 * Update LONG_STRATEGY_ID and SHORT_STRATEGY_ID below before running.
 *
 * Usage: npx tsx --env-file=.env.local scripts/phase5e_run_backtest.ts
 */
...
// UPDATE THESE AFTER RUNNING phase5e_create_strategy.ts:
const LONG_STRATEGY_ID  = process.env.PHASE5E_LONG_ID  ?? "PLACEHOLDER_LONG_ID";
const SHORT_STRATEGY_ID = process.env.PHASE5E_SHORT_ID ?? "PLACEHOLDER_SHORT_ID";

const PHASE5C_TOTAL_TRADES = 2290;  // Phase 5-C baseline
const PHASE5D_TOTAL_TRADES = 1106;  // Phase 5-D baseline
const SYMBOL               = "EURUSD";
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════╗");
  console.log("║    PHASE 5-E: EURUSD Multi-TF EMA21 Pullback v4             ║");
  console.log("║    + RSI(14) State Filter (Overbought/Oversold Exclusion)   ║");
  console.log("║    LONG: RSI < 60  |  SHORT: RSI > 40                      ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  console.log(`\nLONG  strategy ID: ${LONG_STRATEGY_ID}`);
  console.log(`SHORT strategy ID: ${SHORT_STRATEGY_ID}`);

  if (LONG_STRATEGY_ID === "PLACEHOLDER_LONG_ID" || SHORT_STRATEGY_ID === "PLACEHOLDER_SHORT_ID") {
    console.error("\nERROR: Strategy IDs not set. Run phase5e_create_strategy.ts first,");
    console.error("then set PHASE5E_LONG_ID and PHASE5E_SHORT_ID environment variables.");
    process.exit(1);
  }
  ...
  // 2. Run LONG backtest
  console.log("\n[2] Running LONG BacktestEngine (v4 + RSI BELOW 60)...");
  ...
  console.log("\n[3] Running SHORT BacktestEngine (v4 + RSI ABOVE 40)...");
  ...
  // Signal reduction vs Phase 5-C and 5-D
  const signalReductionVs5C = PHASE5C_TOTAL_TRADES > 0
    ? (1 - totalTrades / PHASE5C_TOTAL_TRADES) * 100
    : 0;
  const signalReductionVs5D = PHASE5D_TOTAL_TRADES > 0
    ? ((totalTrades - PHASE5D_TOTAL_TRADES) / PHASE5D_TOTAL_TRADES) * 100
    : 0;
  ...
  // Check RSI state look-ahead: RSI at confirmed bar (not forming)
  // A state filter only reads rsi[idx], where idx = getLastConfirmedBarIndex()
  // Look-ahead violation would mean using bar[idx+1] or forming bar data
  console.log("  RSI state filter look-ahead: Uses only confirmed bar RSI value (no prev bar needed).");
  console.log("  This eliminates the crossover look-ahead concern (no 2-bar window).");
  ...
  // 8. Print full report
  console.log("\n╔══════════════════════════════════════════════════════════════╗");
  console.log("║           PHASE 5-E FINAL BACKTEST REPORT                    ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");

  console.log(`\nStrategy (LONG):  EURUSD Multi-TF EMA21 Pullback v4 LONG`);
  console.log(`Strategy (SHORT): EURUSD Multi-TF EMA21 Pullback v4 SHORT`);
  console.log(`LONG  ID:         ${LONG_STRATEGY_ID}`);
  console.log(`SHORT ID:         ${SHORT_STRATEGY_ID}`);
  console.log(`Symbol:           ${SYMBOL}`);
  console.log(`Main TF:          M5 (entry) + H1/H4 (trend)`);
  console.log(`RSI Filter:       STATE — LONG: RSI(14) < 60  |  SHORT: RSI(14) > 40`);
  console.log(`Filter Type:      STATE (single bar, not crossover)`);
  ...
  console.log(`Avg pips/trade:   ${totalTrades > 0 ? (totalPips / totalTrades).toFixed(1) : "N/A"}`);
  console.log(`Combined PF:      ${combinedPF !== null ? combinedPF.toFixed(2) : "Infinity (no losses)"}`);
  console.log(`LONG max DD:      $${longReport.maxDrawdown} (${longReport.maxDrawdownPct.toFixed(1)}%) / ${longReport.maxDrawdownPips} pips`);
  console.log(`SHORT max DD:     $${shortReport.maxDrawdown} (${shortReport.maxDrawdownPct.toFixed(1)}%) / ${shortReport.maxDrawdownPips} pips`);
  console.log(`Exit TP:          ${exitTP}`);
  console.log(`Exit SL:          ${exitSL}`);
  console.log(`Exit EOD:         ${exitEOD}`);

  console.log("\n── SIGNAL REDUCTION ────────────────────────────────────────────");
  console.log(`Phase 5-C trades: ${PHASE5C_TOTAL_TRADES}`);
  console.log(`Phase 5-D trades: ${PHASE5D_TOTAL_TRADES}`);
  console.log(`Phase 5-E trades: ${totalTrades}`);
  console.log(`Reduction vs 5-C: ${signalReductionVs5C.toFixed(1)}%`);
  const vs5DSign = signalReductionVs5D >= 0 ? "+" : "";
  console.log(`vs Phase 5-D:     ${vs5DSign}${signalReductionVs5D.toFixed(1)}% (${totalTrades > PHASE5D_TOTAL_TRADES ? "more" : "fewer"} trades than 5-D)`);
  ...
  console.log(`LONG max DD%:        ${longReport.maxDrawdownPct.toFixed(1)}%`);
  ...
  console.log(`SHORT max DD%:       ${shortReport.maxDrawdownPct.toFixed(1)}%`);
  console.log(`SHORT max cons losses:${shortReport.maxConsecutiveLosses}`);
  console.log(`SHORT verdict:       ${shortReport.verdict}`);

  console.log("\n── LONG vs SHORT COMPARISON ───────────────────────────────────");
  console.log(`${"Metric".padEnd(22)} ${"LONG".padEnd(15)} SHORT`);
  console.log(`${"─".repeat(52)}`);
  console.log(`${"Trades".padEnd(22)} ${String(longTrades.length).padEnd(15)} ${shortTrades.length}`);
  console.log(`${"Wins".padEnd(22)} ${String(longWins).padEnd(15)} ${shortWins}`);
  console.log(`${"Win Rate".padEnd(22)} ${longWinRate.toFixed(1).padEnd(14)}% ${shortWinRate.toFixed(1)}%`);
  console.log(`${"Total Pips".padEnd(22)} ${longTotalPips.toFixed(1).padEnd(15)} ${shortTotalPips.toFixed(1)}`);
  console.log(`${"Avg Win Pips".padEnd(22)} ${longAvgWin.toFixed(1).padEnd(15)} ${shortAvgWin.toFixed(1)}`);
  console.log(`${"Avg Loss Pips".padEnd(22)} ${longAvgLoss.toFixed(1).padEnd(15)} ${shortAvgLoss.toFixed(1)}`);
  console.log(`${"Max DD %".padEnd(22)} ${longReport.maxDrawdownPct.toFixed(1).padEnd(14)}% ${shortReport.maxDrawdownPct.toFixed(1)}%`);
  console.log(`${"Profit Factor".padEnd(22)} ${(longReport.profitFactor?.toFixed(2) ?? "Inf").padEnd(15)} ${shortReport.profitFactor?.toFixed(2) ?? "Inf"}`);
  console.log(`${"Max Cons Losses".padEnd(22)} ${String(longReport.maxConsecutiveLosses).padEnd(15)} ${shortReport.maxConsecutiveLosses}`);
  ...
  // Phase comparisons
  const phase5cPF = 0.60;
  const phase5dPF = 0.57;
  const pfChangeVs5C  = combinedPF !== null ? combinedPF - phase5cPF : null;
  const pfChangeVs5D  = combinedPF !== null ? combinedPF - phase5dPF : null;
  const pfChangeStr5C = pfChangeVs5C !== null ? (pfChangeVs5C >= 0 ? `+${pfChangeVs5C.toFixed(2)}` : pfChangeVs5C.toFixed(2)) : "N/A";
  const pfChangeStr5D = pfChangeVs5D !== null ? (pfChangeVs5D >= 0 ? `+${pfChangeVs5D.toFixed(2)}` : pfChangeVs5D.toFixed(2)) : "N/A";

  console.log("\n── PHASE 5-C vs 5-D vs 5-E COMPARISON ────────────────────────");
  console.log(`${"Metric".padEnd(25)} ${"Phase 5-C".padEnd(12)} ${"Phase 5-D".padEnd(12)} Phase 5-E`);
  console.log(`${"─".repeat(62)}`);
  console.log(`${"Total Trades".padEnd(25)} ${String(PHASE5C_TOTAL_TRADES).padEnd(12)} ${String(PHASE5D_TOTAL_TRADES).padEnd(12)} ${totalTrades}`);
  console.log(`${"Win Rate".padEnd(25)} ${"N/A".padEnd(12)} ${"N/A".padEnd(12)} ${totalWinRate.toFixed(1)}%`);
  console.log(`${"Profit Factor".padEnd(25)} ${phase5cPF.toFixed(2).padEnd(12)} ${phase5dPF.toFixed(2).padEnd(12)} ${combinedPF !== null ? combinedPF.toFixed(2) : "Inf"}`);
  console.log(`${"PF Change vs 5-C".padEnd(25)} ${"—".padEnd(12)} ${"-0.03".padEnd(12)} ${pfChangeStr5C}`);
  console.log(`${"PF Change vs 5-D".padEnd(25)} ${"—".padEnd(12)} ${"—".padEnd(12)} ${pfChangeStr5D}`);
  console.log(`${"Signal Reduction vs 5-C".padEnd(25)} ${"0%".padEnd(12)} ${"51.7%".padEnd(12)} ${signalReductionVs5C.toFixed(1)}%`);
  console.log(`${"RSI Filter".padEnd(25)} ${"NONE".padEnd(12)} ${"CROSSOVER".padEnd(12)} STATE`);

  // RSI state filter hypothesis verdict
  let rsiStateHypothesis: string;
  if (combinedPF !== null && combinedPF >= 1.15) {
    rsiStateHypothesis = "STRONGLY SUPPORTED";
  } else if (combinedPF !== null && combinedPF >= phase5cPF) {
    rsiStateHypothesis = "SUPPORTED";
  } else if (combinedPF !== null && combinedPF >= phase5dPF) {
    rsiStateHypothesis = "PARTIALLY SUPPORTED (better than 5-D, still below 5-C)";
  } else {
    rsiStateHypothesis = "REJECTED";
  }
  ...
  console.log(`RSI STATE FILTER HYPOTHESIS: ${rsiStateHypothesis}`);

  // RSI state filter effect analysis
  console.log("\n── RSI STATE FILTER EFFECT ANALYSIS ──────────────────────────");
  const filteredOut5C = PHASE5C_TOTAL_TRADES - totalTrades;
  console.log(`Signals removed from 5-C baseline: ${filteredOut5C}`);
  console.log(`LONG filter:  RSI(14) < 60  (excludes overbought LONG entries)`);
  console.log(`SHORT filter: RSI(14) > 40  (excludes oversold SHORT entries)`);
  console.log(`Filter style: STATE — single bar value, no crossover check`);
  console.log(`Prev bar req: NONE (state filter reads only rsi[confirmed_bar_idx])`);
  console.log(`Threshold:    60/40 fixed (no optimization)`);

  console.log("\n╔══════════════════════════════════════════════════════════════╗");
  console.log("║              PHASE 5-E STATUS SUMMARY                        ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  ...
  console.log(`PHASE 5-C PF:                0.60`);
  console.log(`PHASE 5-D PF:                0.57`);
  console.log(`PHASE 5-E PF:                ${combinedPF !== null ? combinedPF.toFixed(2) : "Infinity"}`);
  console.log(`PF CHANGE vs 5-C:            ${pfChangeStr5C}`);
  console.log(`PF CHANGE vs 5-D:            ${pfChangeStr5D}`);
  console.log(`SIGNAL REDUCTION vs 5-C:     ${signalReductionVs5C.toFixed(1)}%`);
  console.log(`SIGNAL REDUCTION vs 5-D:     ${vs5DSign}${signalReductionVs5D.toFixed(1)}% (${totalTrades > PHASE5D_TOTAL_TRADES ? "more" : "fewer"} trades)`);
  console.log(`RSI STATE FILTER HYPOTHESIS: ${rsiStateHypothesis}`);
  console.log(`COMBINED VERDICT:            ${combinedVerdict}`);
  console.log(`LOOK-AHEAD:                  ${lookAheadOK ? "PASS" : "WARNING"}`);
  console.log(`DATA LEAKAGE:                NONE`);
  console.log(`ENGINE FILES CHANGED:        NO`);
  console.log(`OPTIMIZATION:                NOT RUN`);
  console.log(`PARAMETER TUNING:            NOT RUN`);
  console.log(`FILTER TYPE:                 STATE (not crossover)`);
  console.log(`LONG RULE:                   RSI(14) < 60`);
  console.log(`SHORT RULE:                  RSI(14) > 40`);
  ...
  console.log("\n── NEXT PHASE RECOMMENDATION ──────────────────────────────────");
  if (rsiStateHypothesis.includes("STRONGLY SUPPORTED")) {
    console.log("RSI(14) state filter hypothesis STRONGLY SUPPORTED.");
    console.log("Next: Phase 5-F — Explore combining NEAR_EMA threshold tightening with RSI state filter.");
  } else if (rsiStateHypothesis.includes("SUPPORTED")) {
    console.log("RSI(14) state filter hypothesis SUPPORTED.");
    console.log("Next: Phase 5-F — Test volume filter or ATR volatility filter as additional condition.");
  } else if (rsiStateHypothesis.includes("PARTIALLY")) {
    console.log("RSI(14) state filter partially useful (better than 5-D crossover, below 5-C baseline).");
    console.log("Next: Phase 5-F — Test wider RSI bands (BELOW 65 / ABOVE 35) or different indicator.");
  } else {
    console.log("RSI(14) state filter REJECTED. Neither crossover (5-D) nor state (5-E) improves 5-C.");
    console.log("Next: Phase 5-F — Consider different hypothesis: MACD histogram or ADX trend strength.");
  }
}
