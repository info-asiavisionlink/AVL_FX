// scripts/phase5d_create_strategy.ts
// Phase 5-D: Multi-TF EMA21 Pullback + RSI(14) Momentum Confirmation
//
// Creates TWO separate strategy_registry entries (v3 LONG, v3 SHORT)
// Phase 5-C versions (v2) are NOT overwritten.
//
// Change reason: "Added symmetric RSI(14) momentum confirmation (CROSS_UP/DOWN over 50)"
//
// Usage: npx tsx --env-file=.env.local scripts/phase5d_create_strategy.ts
...
async function main() {
  ...
  console.log("=== Phase 5-D Strategy Creation ===\n");
  console.log("Adding RSI(14) momentum confirmation (CROSS_UP/DOWN over 50) to Phase 5-C base.\n");

  // Check existing Phase 5-C strategies (v2) still intact
  for (const name of [
    ...
    if (existing.length > 0) {
      console.log(`Phase 5-C preserved: "${name}" (id=${existing[0].id}, status=${existing[0].backtest_status})`);
    } else {
      console.warn(`WARNING: Phase 5-C strategy "${name}" NOT found in DB`);
    }
  }
  ...
  const longSpec = {
    ...
    description:     "Phase 5-D LONG: EURUSD M5 EMA21 Pullback + RSI(14) momentum — H4+H1 BULLISH trend filter. Added symmetric RSI(14) momentum confirmation (CROSS_UP/DOWN over 50). Base: Phase 5-C v2 LONG.",
    symbols:         ["EURUSD"],
    timeframes:      ["M5", "H1", "H4"],
    entry_conditions: {
      logic: "AND",
  ...
  const shortSpec = {
    ...
    strategy_type:   "DAY_TRADE",
    description:     "Phase 5-D SHORT: EURUSD M5 EMA21 Pullback + RSI(14) momentum — H4+H1 BEARISH trend filter. Added symmetric RSI(14) momentum confirmation (CROSS_UP/DOWN over 50). Base: Phase 5-C v2 SHORT.",
    symbols:         ["EURUSD"],
    timeframes:      ["M5", "H1", "H4"],
    entry_conditions: {
      logic: "AND",
  ...
  console.log("\nNext: update phase5d_run_backtest.ts with these IDs and run it.");
}
