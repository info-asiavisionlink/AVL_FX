/**
 * Phase 5-D Backtest Script
 *
 * Runs TWO strategies (v3 LONG + v3 SHORT) against EURUSD bar data.
 * Phase 5-C base = 2,290 total trades (v2 LONG + v2 SHORT).
 * Phase 5-D adds RSI(14) CROSS_UP/DOWN 50 as a third AND condition.
 *
 * Strategy IDs are set after running phase5d_create_strategy.ts.
...
// UPDATE THESE AFTER RUNNING phase5d_create_strategy.ts:
const LONG_STRATEGY_ID  = process.env.PHASE5D_LONG_ID  ?? "PLACEHOLDER_LONG_ID";
const SHORT_STRATEGY_ID = process.env.PHASE5D_SHORT_ID ?? "PLACEHOLDER_SHORT_ID";

const PHASE5C_TOTAL_TRADES = 2290; // Phase 5-C baseline for signal reduction calculation
const SYMBOL          = "EURUSD";
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════╗");
  console.log("║    PHASE 5-D: EURUSD Multi-TF EMA21 Pullback v3             ║");
  console.log("║    + RSI(14) Momentum Confirmation (CROSS_UP/DOWN 50)       ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  ...
  if (LONG_STRATEGY_ID === "PLACEHOLDER_LONG_ID" || SHORT_STRATEGY_ID === "PLACEHOLDER_SHORT_ID") {
    console.error("\nERROR: Strategy IDs not set. Run phase5d_create_strategy.ts first,");
    console.error("then set PHASE5D_LONG_ID and PHASE5D_SHORT_ID environment variables.");
    process.exit(1);
  }
  ...
  // 2. Run LONG backtest
  console.log("\n[2] Running LONG BacktestEngine (v3 + RSI momentum)...");
  ...
  console.log("\n[3] Running SHORT BacktestEngine (v3 + RSI momentum)...");
  ...
  // Signal reduction vs Phase 5-C
  const signalReductionPct = PHASE5C_TOTAL_TRADES > 0
  ...
  // 8. Print full report
  console.log("\n╔══════════════════════════════════════════════════════════════╗");
  console.log("║           PHASE 5-D FINAL BACKTEST REPORT                    ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");

  console.log(`\nStrategy (LONG):  EURUSD Multi-TF EMA21 Pullback v3 LONG`);
  console.log(`Strategy (SHORT): EURUSD Multi-TF EMA21 Pullback v3 SHORT`);
  console.log(`LONG  ID:         ${LONG_STRATEGY_ID}`);
  console.log(`SHORT ID:         ${SHORT_STRATEGY_ID}`);
  console.log(`Symbol:           ${SYMBOL}`);
  console.log(`Main TF:          M5 (entry) + H1/H4 (trend)`);
  console.log(`Momentum:         RSI(14) CROSS_UP/DOWN 50`);
  ...
  console.log(`Phase 5-D trades: ${totalTrades}`);
  console.log(`Signal reduction: ${signalReductionPct.toFixed(1)}%`);
  console.log(`(RSI CROSS_UP/DOWN filter removed ${PHASE5C_TOTAL_TRADES - totalTrades} signals)`);
  ...
  // Phase 5-C vs 5-D comparison (PF = 0.60 from Phase 5-C)
  const phase5cPF = 0.60;
  const pfChange  = combinedPF !== null ? combinedPF - phase5cPF : null;
  const pfChangeStr = pfChange !== null ? (pfChange >= 0 ? `+${pfChange.toFixed(2)}` : pfChange.toFixed(2)) : "N/A";
  ...
  console.log("\n── PHASE 5-C vs 5-D COMPARISON ────────────────────────────────");
  console.log(`${"Metric".padEnd(25)} ${"Phase 5-C".padEnd(12)} Phase 5-D`);
  console.log(`${"─".repeat(50)}`);
  console.log(`${"Total Trades".padEnd(25)} ${String(PHASE5C_TOTAL_TRADES).padEnd(12)} ${totalTrades}`);
  console.log(`${"Win Rate".padEnd(25)} ${"N/A (total)".padEnd(12)} ${totalWinRate.toFixed(1)}%`);
  console.log(`${"Profit Factor".padEnd(25)} ${phase5cPF.toFixed(2).padEnd(12)} ${combinedPF !== null ? combinedPF.toFixed(2) : "Inf"}`);
  console.log(`${"PF Change".padEnd(25)} ${"—".padEnd(12)} ${pfChangeStr}`);
  console.log(`${"Signal Reduction".padEnd(25)} ${"0%".padEnd(12)} ${signalReductionPct.toFixed(1)}%`);

  // Momentum hypothesis verdict
  let momentumHypothesis: string;
  if (combinedPF !== null && combinedPF >= 1.15) {
    momentumHypothesis = "STRONGLY SUPPORTED";
  } else if (combinedPF !== null && combinedPF >= phase5cPF) {
    momentumHypothesis = "SUPPORTED";
  } else {
    momentumHypothesis = "REJECTED";
  }

  console.log("\n╔══════════════════════════════════════════════════════════════╗");
  console.log("║              PHASE 5-D STATUS SUMMARY                        ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  ...
  console.log(`TOTAL trades:             ${totalTrades}`);
  console.log(`Signal reduction:         ${signalReductionPct.toFixed(1)}% vs Phase 5-C`);
  console.log(`LONG  trades:             ${longTrades.length}`);
  console.log(`SHORT trades:             ${shortTrades.length}`);
  console.log(`LONG  win rate:           ${longWinRate.toFixed(2)}%`);
  console.log(`SHORT win rate:           ${shortWinRate.toFixed(2)}%`);
  ...
  console.log(`PHASE 5-C PF:             ${phase5cPF}`);
  console.log(`PF change:                ${pfChangeStr}`);
  console.log(`MOMENTUM HYPOTHESIS:      ${momentumHypothesis}`);
  ...
  console.log(`WALK FORWARD:             NOT RUN`);
  console.log(`MONTE CARLO:              NOT RUN`);
